...
const ENTITY_REQUEST_TAXONOMY_DEFAULTS = [
...
function getTenantRequestTaxonomy(tenant = null) {
  ...
  return ENTITY_REQUEST_TAXONOMY_DEFAULTS.map((base) => {
    const item = byKey.get(base.key) || {};
    const classifications = Array.isArray(item.classifications) && item.classifications.length ? item.classifications.map((x) => String(x || '').trim()).filter(Boolean) : base.classifications;
...
function buildEntityRequestTaxonomyFromBody(body = {}, tenantTaxonomy = []) {
  return tenantTaxonomy.map((base) => {
    const selected = parseSelectedValues(body[`taxonomyClassifications_${base.key}`]);
    const allowed = new Set((base.classifications || []).map((item) => String(item)));
    return {
      key: base.key,
      label: base.label,
      enabled: body[`taxonomyEnabled_${base.key}`] === 'on',
      classificationRequired: body[`taxonomyClassificationRequired_${base.key}`] === 'on',
      classifications: selected.filter((item) => allowed.has(item))
    };
  });
}

function mergeEntityRequestTaxonomy(entityTaxonomy = [], tenantTaxonomy = []) {
  const byKey = new Map((Array.isArray(entityTaxonomy) ? entityTaxonomy : []).map((item) => [String(item.key || '').toUpperCase(), item]));
  return tenantTaxonomy.map((base) => {
    const item = byKey.get(base.key) || {};
...
async function listEntities(req, res, next) {
  try {
    ...
    const clientUserCounts = await UserEntityMembership.aggregate([
      { $match: { tenantId: req.tenant._id, status: 'active' } },
      { $lookup: { from: 'users', localField: 'userId', foreignField: '_id', as: 'user' } },
      { $unwind: '$user' },
      { $match: { 'user.role': { $in: ['client', 'client_user'] }, 'user.isActive': { $ne: false } } },
      { $group: { _id: '$entityId', count: { $sum: 1 } } }
    ]);
    const clientUserCountMap = new Map(clientUserCounts.map((row) => [String(row._id), Number(row.count || 0)]));
    const entityMap = new Map(entities.map((entity) => [String(entity._id), { ...(entity.toObject ? entity.toObject() : entity), children: [], clientUserCount: clientUserCountMap.get(String(entity._id)) || 0 }]));
    const roots = [];
    entityMap.forEach((entity) => {
      const parentId = String(entity.parentId?._id || entity.parentId || '');
      if (parentId && entityMap.has(parentId)) entityMap.get(parentId).children.push(entity);
      else roots.push(entity);
    });
    const sortTree = (items = []) => items.sort((a, b) => String(a.path || a.name).localeCompare(String(b.path || b.name))).forEach((item) => sortTree(item.children || []));
    sortTree(roots);
    const { page, pageSize } = getPagination(req.query, 10);
    const totalItems = roots.length;
    const pager = buildPager({ totalItems, page, pageSize });
    return res.render('entities/list', { title: 'Entities', entities, entityTreeRoots: roots, filters, pager, buildQueryString });
...
async function showCreateEntity(req, res, next) {
  try {
    ...
    let defaults = { type: req.currentUser.role === 'client' ? 'subclient' : (req.query.type || 'client'), parentId: req.query.parentId || '', name: '', acronym: '', region: '', product: '', productIds: [], moduleIds: [], defaultSlaPolicyId: '', defaultOlaPolicyId: '', inheritCommitments: true, jiraEnabled: false, jiraProjectKey: '', jiraProjectId: '', jiraIssueTypeId: '', jiraIssueTypeName: '', jiraAutoPushOnCreate: false, jiraInheritFromParent: true, requestTaxonomy: getTenantRequestTaxonomy(tenantSettings) };
    ...
    return res.render('entities/new', { title: 'Create Client / Sub-client', formMode: 'create', formAction: `${req.basePath}/entities`, formMethod: 'POST', methodOverride: null, parents, defaults, jiraProjects, products, modules, policies, regionOptions, requestTaxonomy: getTenantRequestTaxonomy(tenantSettings), entity: null });
  } catch (error) {
    return next(error);
  }
}
...
async function createEntity(req, res, next) {
  try {
    let { name, acronym, type, parentId, region, product } = req.body;
    ...
    if (req.currentUser.role === 'client') {
      if (type !== 'subclient') {
        req.session.error = 'Client users can only create sub-clients.';
        return res.redirect(`${req.basePath}/entities/new`);
      }
      if (!(await userHasEntityAccess(req.currentUser, parentId))) {
        req.session.error = 'You do not have access to the selected parent client.';
        return res.redirect(`${req.basePath}/entities/new`);
      }
    }

    if (parentId) type = 'subclient';

    const tenantSettings = await Tenant.findById(req.tenant._id).lean();
    const tenantTaxonomy = getTenantRequestTaxonomy(tenantSettings);
    ...
    const entity = await createEntityForTenant({
      ...
      metadata: { region, product, productIds: parseSelectedValues(req.body.productIds), moduleIds: parseSelectedValues(req.body.moduleIds), requestTaxonomy: buildEntityRequestTaxonomyFromBody(req.body, tenantTaxonomy) },
...
async function showEditEntity(req, res, next) {
  try {
    ...
    const defaults = {
      ...
      moduleIds: (entity.metadata?.moduleIds || []).map((item) => String(item)),
      ...
      requestTaxonomy: mergeEntityRequestTaxonomy(entity.metadata?.requestTaxonomy, getTenantRequestTaxonomy(tenantSettings))
    };
    return res.render('entities/new', { title: `Edit ${entity.name}`, formMode: 'edit', formAction: `${req.basePath}/entities/${entity._id}/edit`, formMethod: 'POST', methodOverride: null, parents, defaults, jiraProjects, products, modules, policies, regionOptions, requestTaxonomy: getTenantRequestTaxonomy(tenantSettings), entity });
  } catch (error) { return next(error); }
}
  ...
  try {
    ...
    const entity = await updateEntityForTenant({
      ...
      updates: {
        ...
        moduleIds: parseSelectedValues(req.body.moduleIds),
        requestTaxonomy: buildEntityRequestTaxonomyFromBody(req.body, tenantTaxonomy),
...
async function changeEntityStatus(req, res, next) {
  try {
    const willActivate = String(req.body.isActive) === 'true' || req.body.action === 'activate';
    if (!willActivate) {
      const expected = String(req.body.confirmationExpected || '').trim().toUpperCase();
      const typed = String(req.body.confirmationCode || '').trim().toUpperCase();
      if (!expected || expected !== typed) {
        req.session.error = 'Deactivate confirmation code did not match.';
        return res.redirect(`${req.basePath}/entities`);
      }
    }
    const entity = await setEntityActiveState({ tenantId: req.tenant._id, entityId: req.params.id, isActive: willActivate });
...
async function createEntityApi(req, res, next) {
  try {
    ...
    if (req.currentUser.role === 'client') {
      if (type !== 'subclient') return res.status(403).json({ error: 'Client users can only create sub-clients.' });
      if (!(await userHasEntityAccess(req.currentUser, parentId))) {
        return res.status(403).json({ error: 'You do not have access to the selected parent client.' });
      }
    }
