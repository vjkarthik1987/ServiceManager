...
function sanitizeAcronym(acronym = '') {
  return String(acronym || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 6);
}
...
async function resolveUniqueAcronym({ tenantId, name, requestedAcronym = '', excludeEntityId = null }) {
  ...
  if (!base || base.length !== 6) {
    const err = new Error('Acronym must be exactly 6 letters or digits and unique within the tenant.');
    err.status = 400;
    throw err;
  }
  ...
  for (let i = 1; i <= 999; i += 1) {
    ...
    const candidate = `${base.slice(0, Math.max(1, 6 - suffix.length))}${suffix}`.slice(0, 6);
    if (!existingAcronyms.has(candidate)) return candidate;
  }
  const err = new Error('Unable to auto-generate a unique 6-character acronym. Please enter one manually.');
  err.status = 409;
  throw err;
}
...
async function createEntityForTenant({ tenantId, name, acronym = '', type, parentId = null, metadata = {}, jiraConfig = {}, commitmentDefaults = {} }) {
  ...
  const entity = await Entity.create({
    ...
    metadata: {
      ...
      moduleIds: Array.isArray(metadata.moduleIds) ? metadata.moduleIds : [],
...
async function updateEntityForTenant({ tenantId, entityId, updates = {} }) {
  ...
  entity.metadata = {
    ...
    productIds: Array.isArray(updates.productIds) ? updates.productIds : (Array.isArray(updates.metadata?.productIds) ? updates.metadata.productIds : (entity.metadata?.productIds || [])),
    moduleIds: Array.isArray(updates.moduleIds) ? updates.moduleIds : (Array.isArray(updates.metadata?.moduleIds) ? updates.metadata.moduleIds : (entity.metadata?.moduleIds || [])),
