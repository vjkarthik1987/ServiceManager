...
async function saveJiraConnection(req, res, next) {
  try {
    const existingConnection = await getTenantJiraConnection({
      tenantId: req.tenant._id,
      includeSecret: true
    });

    if (!req.body.baseUrl || !req.body.email || (!existingConnection && !req.body.apiToken)) {
      const message = existingConnection
        ? 'baseUrl and email are required.'
