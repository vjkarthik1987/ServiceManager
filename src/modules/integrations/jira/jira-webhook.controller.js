...
const READY_TO_CLOSE_KEYWORDS = ['READY TO CLOSE', 'READY_TO_CLOSE', 'CLOSE', 'CLOSED', 'DONE', 'COMPLETE', 'COMPLETED', 'FINISH', 'FINISHED'];
const RESOLVED_KEYWORDS = ['RESOLV'];
const WAITING_FOR_CLIENT_KEYWORDS = ['WAIT', 'PENDING CUSTOMER', 'NEED INFO', 'MORE INFO', 'ON HOLD'];
const IN_PROGRESS_KEYWORDS = ['PROGRESS', 'ACTIVE', 'ONGOING', 'IN DEVELOPMENT', 'IN REVIEW', 'IMPLEMENT', 'TEST'];
const OPEN_KEYWORDS = ['OPEN', 'TODO', 'TO DO', 'BACKLOG', 'SELECTED FOR DEVELOPMENT', 'TRIAGE'];
...
function mapWebhookStatus(raw) {
  ...
  if (containsAnyKeyword(value, READY_TO_CLOSE_KEYWORDS)) return 'READY_TO_CLOSE';
  if (containsAnyKeyword(value, RESOLVED_KEYWORDS)) return 'RESOLVED';
  if (containsAnyKeyword(value, WAITING_FOR_CLIENT_KEYWORDS)) return 'WAITING_FOR_CLIENT';
  if (containsAnyKeyword(value, IN_PROGRESS_KEYWORDS)) return 'IN_PROGRESS';
  if (containsAnyKeyword(value, OPEN_KEYWORDS)) return 'OPEN';
  return 'OPEN';
}
...
function getCurrentJiraStatusInfo(body = {}) {
  ...
  const name = String(
    ...
    statusChange?.toStringValue ||
    statusChange?.to ||
    issueStatus?.name ||
    body?.transition?.to_status ||
    body?.status ||
    body?.issue?.fields?.status?.statusCategory?.name ||
    ''
  ).trim();
  ...
  const category = String(
    ...
    body?.issue?.fields?.status?.statusCategory?.key ||
    body?.issue?.fields?.status?.statusCategory?.name ||
    ''
  ).trim();
...
async function receiveJiraWebhook(req, res, next) {
  try {
    ...
    try {
      ...
      if (error?.code === 11000) {
        duplicateEvent = true;
        await JiraWebhookEvent.updateOne(
          { tenantId, $or: [{ eventId }, ...(fingerprint ? [{ fingerprint }] : [])] },
          { $set: { processedAt: new Date(), payload: req.body, issueKey } }
        ).catch(() => null);
      } else {
        throw error;
      }
    ...
    const issue = await Issue.findOne({
      tenantId,
      $or: [
        ...(normalizedIssueKey ? [{ 'jira.issueKey': normalizedIssueKey }] : []),
    ...
    issue.jira.currentStatusName = jiraStatus.name || rawStatus || issue.jira.currentStatusName || '';
    ...
    if (mappedStatus === 'READY_TO_CLOSE') {
      ...
      await createNotification({ tenantId, issueId: issue._id, type: 'AGENT_CLOSURE_REQUIRED', recipientUserId: issue.assignedToUserId || null, subject: `Agent closure required for ${issue.issueNumber}`, body: `Jira issue ${issueKey} is in a closed/completed state and Service Desk is now marked Closed for Review.` });
    }
    ...
    return res.json({ ok: true, duplicate: duplicateEvent, issueId: String(issue._id), mappedStatus, jiraStatusName: issue.jira.currentStatusName || '', jiraStatusCategory: issue.jira.currentStatusCategory || '' });
  } catch (error) {
    return next(error);
  }
}
