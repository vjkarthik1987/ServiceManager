...
async function saveTenantJiraConnection({ tenantId, payload }) {
  ...
  return getTenantJiraConnection({ tenantId, includeSecret: true });
}
...
async function validateTenantJiraConnection({ tenantId }) {
  ...
  try {
    if (!connection.apiToken || String(connection.apiToken).trim().length < 10) {
      throw new Error('Jira API token is missing or too short. Re-save Jira configuration with a valid API token.');
    }
