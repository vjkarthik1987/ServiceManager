...
async function enqueueJiraPush({ tenantId, issueId, triggeredByUserId = null, requestedProjectKey = '', issueTypeId = '', issueTypeName = '' }) {
  ...
  // v30.80: default to immediate processing so manual pushes do not sit as QUEUED
  // when the embedded worker is disabled. Set JIRA_PUSH_ASYNC=true to restore pure async queue mode.
  if (isMockMode() || String(process.env.JIRA_PUSH_ASYNC || 'false').toLowerCase() !== 'true') {
