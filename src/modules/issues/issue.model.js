...
const ISSUE_STATUSES = ['NEW', 'OPEN', 'UNDER_REVIEW', 'APPROVED', 'IN_PROGRESS', 'WAITING_FOR_CLIENT', 'ANSWERED', 'UAT', 'RESOLVED', 'READY_TO_CLOSE', 'REOPENED', 'CLOSED'];
...
const issueSchema = new mongoose.Schema(
  {
    ...
    priority: { type: String, default: 'MEDIUM', trim: true, index: true },
    ...
    assignmentMode: { type: String, enum: ['UNASSIGNED', 'PUSH', 'PULL'], default: 'UNASSIGNED' },
    ...
    intake: {
      affectedUsers: { type: String, default: '', trim: true },
      businessImpact: { type: String, default: '', trim: true },
      isBlocking: { type: String, enum: ['', 'YES', 'NO', 'PARTIAL'], default: '' },
      workaroundAvailable: { type: String, enum: ['', 'YES', 'NO', 'UNKNOWN'], default: '' },
      environment: { type: String, default: '', trim: true },
      startedAtText: { type: String, default: '', trim: true },
    ...
    ai: {
      ...
      similarIssueRefs: { type: [mongoose.Schema.Types.Mixed], default: [] },
      confidence: { type: Number, default: null },
      lastSuggestedAt: { type: Date, default: null },
      feedback: { type: [mongoose.Schema.Types.Mixed], default: [] }
    },
    requestType: { type: String, enum: ['BUG', 'CR', 'QUERY', 'SERVICE_REQUEST'], default: 'BUG', index: true },
    requestClassification: { type: String, default: '', trim: true, index: true },
    ...
    sla: {
      ...
      permanentFixTargetMinutes: { type: Number, default: null },
      permanentFixDueAt: { type: Date, default: null },
      permanentFixStatus: { type: String, enum: ['NO_SLA', 'ON_TRACK', 'AT_RISK', 'MET', 'BREACHED'], default: 'NO_SLA', index: true },
    ...
    closure: {
      ...
      closureSource: { type: String, enum: ['', 'CLIENT_DIRECT', 'AGENT', 'SUPERADMIN', 'SYSTEM'], default: '' },
      rca: {
        ...
        customerClosureNote: { type: String, default: '', trim: true },
    ...
    source: { type: String, default: 'portal', trim: true },
...
issueSchema.index({ tenantId: 1, category: 1, product: 1, module: 1, priority: 1, updatedAt: -1 });
