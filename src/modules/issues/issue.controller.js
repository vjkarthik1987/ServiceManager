...
const { canCreateIssue, isClientUser, isInternalAgent, isEngagementManager, canAssign, canTriage, canTakeOwnership } = require('../../utils/roles');
...
const REQUEST_TYPES = ['BUG', 'CR', 'QUERY', 'SERVICE_REQUEST'];
...
async function createKbDraftFromClosedIssue({ tenantId, issue, userId }) {
  try {
    if (!issue || !issue.closure || !issue.closure.rca) return;
    const rca = issue.closure.rca || {};
    const usefulText = [issue.title, issue.description, rca.rootCause, rca.resolutionSummary, rca.preventiveAction].filter(Boolean).join('\n\n');
    if (usefulText.trim().length < 40) return;
    const existing = await KnowledgeDocument.findOne({ tenantId, sourceIssueId: issue._id, sourceType: 'AUTO_KB_DRAFT' }).lean();
    if (existing) return;
    await KnowledgeDocument.create({
      tenantId,
      uploadedByUserId: userId,
      title: `KB draft - ${issue.issueNumber} - ${issue.title}`.slice(0, 180),
      sourceType: 'AUTO_KB_DRAFT',
      sourceIssueId: issue._id,
      visibilityScope: 'INTERNAL_ONLY',
      originalName: issue.issueNumber,
      rawText: [
        `Issue: ${issue.issueNumber}`,
        `Title: ${issue.title}`,
        `Symptoms: ${issue.description}`,
        `Root cause: ${rca.rootCause || '-'}`,
        `Resolution: ${rca.resolutionSummary || '-'}`,
      ...
      confidence: 0.72,
      tags: [issue.category, issue.product, issue.priority, 'auto-kb'].filter(Boolean),
      metadata: { generatedFrom: 'issue_closure', issueNumber: issue.issueNumber }
    });
  } catch (error) {
    console.error('Auto KB draft creation failed', error.message);
  }
}

function deferWork(label, work) {
  setImmediate(async () => {
    try {
      await work();
    } catch (error) {
...
function getTenantIssueConfig(tenant = null) {
  return {
    priorities: normalizeMasterList(tenant?.issueConfig?.priorities, ISSUE_PRIORITIES),
    categories: normalizeMasterList(tenant?.issueConfig?.categories, DEFAULT_ISSUE_CATEGORIES),
    sources: normalizeMasterList(tenant?.issueConfig?.sources, DEFAULT_ISSUE_SOURCES),
    requestTypes: getEnabledRequestTypes(tenant),
    requestTaxonomy: getRequestTaxonomy(tenant),
    jiraIssueTypeByRequestType: {
      bug: String(tenant?.issueConfig?.jiraIssueTypeByRequestType?.bug || 'Bug').trim() || 'Bug',
      cr: String(tenant?.issueConfig?.jiraIssueTypeByRequestType?.cr || 'Change Request').trim() || 'Change Request',
      query: String(tenant?.issueConfig?.jiraIssueTypeByRequestType?.query || 'Task').trim() || 'Task',
      serviceRequest: String(tenant?.issueConfig?.jiraIssueTypeByRequestType?.serviceRequest || 'Service Request').trim() || 'Service Request'
    }
  };
}
...
function getRequestTaxonomy(tenant = null) {
  ...
  return DEFAULT_REQUEST_TAXONOMY.map((base) => {
    ...
    return {
      ...
      classificationRequired: Boolean(item.classificationRequired),
      classifications
    };
  });
}
function getEnabledRequestTypes(tenant = null) {
  return getRequestTaxonomy(tenant).filter((item) => item.enabled !== false).map((item) => item.key);
}
function resolveRequestClassification({ tenant = null, requestType = 'BUG', category = '' } = {}) {
  const taxonomy = getRequestTaxonomy(tenant).find((item) => item.key === requestType) || null;
  const raw = String(category || '').trim();
  if (!taxonomy) return raw;
  if (!taxonomy.classifications.length) return taxonomy.label;
  const match = taxonomy.classifications.find((item) => item.toLowerCase() === raw.toLowerCase());
  if (match) return match;
  if (taxonomy.classificationRequired) return '';
  return raw || taxonomy.classifications[0] || taxonomy.label;
}


function mergeEntityRequestTaxonomy(entity = null, tenant = null) {
  const tenantTaxonomy = getRequestTaxonomy(tenant);
  const entityTaxonomy = Array.isArray(entity?.metadata?.requestTaxonomy) ? entity.metadata.requestTaxonomy : [];
  if (!entityTaxonomy.length) return tenantTaxonomy;
  const byKey = new Map(entityTaxonomy.map((item) => [String(item.key || '').toUpperCase(), item]));
  return tenantTaxonomy.map((base) => {
    const item = byKey.get(base.key) || {};
    const selected = Array.isArray(item.classifications) ? item.classifications.map((x) => String(x || '').trim()).filter(Boolean) : base.classifications;
    const allowed = new Set((base.classifications || []).map((x) => String(x)));
    return {
      key: base.key,
      label: base.label,
      enabled: typeof item.enabled === 'boolean' ? item.enabled : base.enabled,
      classificationRequired: Boolean(item.classificationRequired),
      classifications: selected.filter((x) => allowed.has(x))
    };
  });
}
function getEnabledRequestTypesForEntity(entity = null, tenant = null) {
  return mergeEntityRequestTaxonomy(entity, tenant).filter((item) => item.enabled !== false).map((item) => item.key);
}
function resolveRequestTypeForEntity(value = '', fallback = 'BUG', tenant = null, entity = null) {
  const normalized = String(value || fallback || 'BUG').trim().toUpperCase();
  const enabled = getEnabledRequestTypesForEntity(entity, tenant);
  return REQUEST_TYPES.includes(normalized) && enabled.includes(normalized) ? normalized : '';
}
function resolveRequestClassificationForEntity({ tenant = null, entity = null, requestType = 'BUG', category = '' } = {}) {
  const taxonomy = mergeEntityRequestTaxonomy(entity, tenant).find((item) => item.key === requestType) || null;
  if (!taxonomy) return String(category || '').trim();
  if (!taxonomy.classificationRequired || !taxonomy.classifications.length) return taxonomy.label;
  const raw = String(category || '').trim();
  const match = taxonomy.classifications.find((item) => item.toLowerCase() === raw.toLowerCase());
  return match || '';
}
...
function matchConfiguredOption(value = '', options = []) {
  ...
  if (!raw) return '';
  return options.find((item) => String(item || '').trim().toLowerCase() === raw.toLowerCase()) || '';
}

function resolveConfiguredPriority(value = '', tenant = null, fallback = '') {
  const masters = getTenantIssueConfig(tenant);
  return matchConfiguredOption(value || fallback, masters.priorities);
}

function resolveConfiguredCategory(value = '', tenant = null, fallback = '') {
  const masters = getTenantIssueConfig(tenant);
  return matchConfiguredOption(value || fallback, masters.categories);
}

function resolveConfiguredSource(value = '', tenant = null, fallback = 'portal') {
  const masters = getTenantIssueConfig(tenant);
  return matchConfiguredOption(value || fallback, masters.sources);
}

function resolveRequestType(value = '', fallback = 'BUG', tenant = null) {
  const normalized = String(value || fallback || 'BUG').trim().toUpperCase();
  const enabled = getEnabledRequestTypes(tenant);
  return REQUEST_TYPES.includes(normalized) && enabled.includes(normalized) ? normalized : '';
}

function getJiraIssueTypeForRequestType(tenant = null, requestType = 'BUG') {
  const config = getTenantIssueConfig(tenant);
  if (requestType === 'CR') return config.jiraIssueTypeByRequestType.cr;
  if (requestType === 'QUERY') return config.jiraIssueTypeByRequestType.query;
  if (requestType === 'SERVICE_REQUEST') return config.jiraIssueTypeByRequestType.serviceRequest || 'Service Request';
  return config.jiraIssueTypeByRequestType.bug;
}
...
async function notifyIssueStakeholders({ tenantId, issue, type, actorUserId = null, subject = '', body = '', extraUserIds = [] }) {
  try {
    ...
    const actorUser = actorUserId ? await User.findById(actorUserId).select('name email role').lean().catch(() => null) : null;
    const metadata = {
      ...
      severity: issue.priority || issue.severity || '',
      priority: issue.priority || '',
      product: Array.isArray(issue.products) ? issue.products.join(', ') : (issue.product || ''),
      category: issue.category || '',
      entityName: issue.entityId?.name || '',
      entityPath: issue.entityId?.path || '',
      assignedToName: issue.assignedToUserId?.name || issue.assignedToUserId?.email || '',
      actorName: actorUser?.name || actorUser?.email || ''
    };
    await notifyUsers({ tenantId, issueId: issue._id, type, userIds: stakeholderIds, actorUserId, subject, body, metadata, sendEmail: true });
  } catch (error) {
...
const READY_TO_CLOSE_JIRA_KEYWORDS = ['READY TO CLOSE', 'READY_TO_CLOSE', 'CLOSE', 'CLOSED', 'DONE', 'COMPLETE', 'COMPLETED', 'FINISH', 'FINISHED'];
const RESOLVED_JIRA_KEYWORDS = ['RESOLV'];
const WAITING_FOR_CLIENT_JIRA_KEYWORDS = ['WAIT', 'PENDING CUSTOMER', 'NEED INFO', 'MORE INFO', 'ON HOLD'];
const IN_PROGRESS_JIRA_KEYWORDS = ['PROGRESS', 'ACTIVE', 'ONGOING', 'IN DEVELOPMENT', 'IN REVIEW', 'IMPLEMENT', 'TEST'];
const OPEN_JIRA_KEYWORDS = ['OPEN', 'TODO', 'TO DO', 'BACKLOG', 'SELECTED FOR DEVELOPMENT', 'TRIAGE'];

function containsJiraKeyword(value = '', keywords = []) {
...
function mapJiraStatusToEsop(raw) {
  ...
  if (containsJiraKeyword(value, READY_TO_CLOSE_JIRA_KEYWORDS)) return 'READY_TO_CLOSE';
  if (containsJiraKeyword(value, RESOLVED_JIRA_KEYWORDS)) return 'RESOLVED';
  if (containsJiraKeyword(value, WAITING_FOR_CLIENT_JIRA_KEYWORDS)) return 'WAITING_FOR_CLIENT';
  if (containsJiraKeyword(value, IN_PROGRESS_JIRA_KEYWORDS)) return 'IN_PROGRESS';
  if (containsJiraKeyword(value, OPEN_JIRA_KEYWORDS)) return 'OPEN';
  return 'OPEN';
}
...
function deriveEsopStatusFromJira({ jiraStatusName = '', jiraStatusCategory = '', fallbackStatus = 'OPEN' } = {}) {
  ...
  if (['new', 'to-do', 'todo'].includes(normalizedCategory)) return 'OPEN';
  if (['indeterminate', 'in-progress', 'in progress', 'active'].includes(normalizedCategory)) return 'IN_PROGRESS';
...
function syncIssueStateFromJiraStatus(issue, jiraStatusName = '', jiraStatusCategory = '', source = 'MANUAL_REFRESH') {
  ...
  issue.markModified('jira');
  issue.markModified('closure');
  issue.markModified('sla');
...
function queryValues(value) {
  const raw = Array.isArray(value) ? value : (typeof value === 'undefined' || value === null ? [] : [value]);
  return raw
...
function applyMultiFilter(filter, field, values) {
  if (!values || !values.length) return;
  filter[field] = values.length === 1 ? values[0] : { $in: values };
}

function applyEntityMultiFilter(filter, values) {
  if (!values || !values.length) return;
  const selected = values.map(String);
  if (filter.entityId && filter.entityId.$in) {
    const scoped = filter.entityId.$in.map((item) => String(item));
    filter.entityId = { $in: selected.filter((item) => scoped.includes(item)) };
    return;
  }
  filter.entityId = values.length === 1 ? values[0] : { $in: values };
}
...
function getReporterTypeForUser(user) {
  ...
  if (isEngagementManager(user?.role)) return 'system';
  return 'system';
}

function canChangeIssueStatus(user) {
  return ['client', 'agent', 'agent_user', 'agent_manager', 'superadmin'].includes(user.role);
}

function canAssignIssue(user) {
  return canAssign(user?.role);
}

function canTriageIssue(user) {
  return canTriage(user?.role);
}

function canChangeExecutionMode(user) {
  return ['agent', 'agent_user', 'agent_manager', 'superadmin'].includes(user.role);
}
...
function shouldMarkFirstResponse(issue, user, visibility) {
  if (String(visibility || 'EXTERNAL').toUpperCase() !== 'EXTERNAL') return false;
  const actorIsClient = ['client', 'client_user'].includes(user?.role);
  const creatorRole = issue?.createdByUserId?.role || issue?.createdByUserId?.toObject?.()?.role || '';
  const createdByClient = ['client', 'client_user'].includes(creatorRole);
  if (createdByClient) return ['agent', 'agent_user', 'agent_manager', 'superadmin'].includes(user?.role);
  return actorIsClient || ['agent', 'agent_user', 'agent_manager', 'superadmin'].includes(user?.role);
}
...
async function syncSlaForIssue(issue, { changedStatus = false, commentActor = null, commentVisibility = null } = {}) {
  ...
  if (commentActor && !issue.sla.firstRespondedAt && shouldMarkFirstResponse(issue, commentActor, commentVisibility)) {
    issue.sla.firstRespondedAt = new Date();
    issue.sla.respondedByUserId = commentActor._id;
  }
  if (changedStatus) {
    if (['WAITING_FOR_CLIENT','RESOLVED','READY_TO_CLOSE','CLOSED'].includes(issue.status) && !issue.sla.firstRespondedAt) {
      issue.sla.firstRespondedAt = new Date();
    }
...
function pushStageTracker(issue, fromStatus, toStatus) {
  if (!issue?.sla) return;
  const now = new Date();
  if (!Array.isArray(issue.sla.stageStatus)) issue.sla.stageStatus = [];
  const active = issue.sla.stageStatus.find((item) => item && !item.exitedAt);
  if (active) {
    active.exitedAt = now;
    active.durationMinutes = Math.max(0, Math.round((now.getTime() - new Date(active.enteredAt || now).getTime()) / 60000));
    active.excludeFromFinalSla = String(active.status || '') === 'WAITING_FOR_CLIENT';
  }
  issue.sla.stageStatus.push({ status: toStatus, fromStatus: fromStatus || '', enteredAt: now, exitedAt: null, durationMinutes: 0, excludeFromFinalSla: String(toStatus || '') === 'WAITING_FOR_CLIENT' });
}
...
function normalizeAttachment(attachment = {}, tenantSlug = "") {
  ...
  return {
    ...
    visibility: attachment.visibility || (attachment.isInternalOnly ? 'INTERNAL' : 'EXTERNAL'),
    isInternalOnly: !!attachment.isInternalOnly || attachment.visibility === 'INTERNAL',
    previewKind,
...
async function uploadFilesAndBuildAttachments({ files = [], tenantId, issueId, uploadedBy, commentId = null, entityId = null, visibility = 'EXTERNAL', isInternalOnly = false }) {
  const assets = await Promise.all((Array.isArray(files) ? files : []).map((file) => uploadFile({ file, tenantId, issueId, uploadedBy, commentId, entityId, visibility, isInternalOnly })));
  return assets.map((asset) => ({
    ...
    visibility: asset.visibility || (asset.isInternalOnly ? 'INTERNAL' : 'EXTERNAL'),
    isInternalOnly: !!asset.isInternalOnly,
...
function getCommentVisibilityFilter(user) {
  if (['client','client_user'].includes(user.role)) return { visibility: 'EXTERNAL' };
  return {};
}

function getCommentFormOptions(user) {
  return {
    canComment: ['client','client_user','agent','agent_user','agent_manager','engagement_manager','superadmin'].includes(user.role),
    canChooseVisibility: ['agent','agent_user','agent_manager','engagement_manager','superadmin'].includes(user.role),
...
function canUserViewIssue(user, issue) {
  ...
  if (['agent','agent_user','agent_manager','engagement_manager','superadmin'].includes(user.role)) return true;
...
function applyCustomerVisibilityFilter(filter, user) {
  if (user && ['client','client_user'].includes(user.role)) {
    filter.$and = [
      ...(filter.$and || []),
      {
        $or: [
    ...
    delete filter.customerVisibility;
  }
  return filter;
}

function getHumanCustomerStatus(issue, workflow = null) {
  return getClientStatusPresentation(workflow, issue.status).clientLabel;
}

function getAllowedStatusTransitions(user, currentStatus) {
  const internalTransitions = {
    NEW: ['OPEN', 'UNDER_REVIEW', 'IN_PROGRESS', 'WAITING_FOR_CLIENT', 'RESOLVED', 'CLOSED'],
    OPEN: ['UNDER_REVIEW', 'IN_PROGRESS', 'WAITING_FOR_CLIENT', 'ANSWERED', 'RESOLVED', 'READY_TO_CLOSE', 'CLOSED'],
    UNDER_REVIEW: ['APPROVED', 'IN_PROGRESS', 'WAITING_FOR_CLIENT', 'ANSWERED', 'RESOLVED'],
    APPROVED: ['IN_PROGRESS', 'WAITING_FOR_CLIENT', 'RESOLVED'],
    IN_PROGRESS: ['WAITING_FOR_CLIENT', 'ANSWERED', 'UAT', 'RESOLVED', 'OPEN', 'READY_TO_CLOSE', 'CLOSED'],
    WAITING_FOR_CLIENT: ['IN_PROGRESS', 'ANSWERED', 'RESOLVED', 'OPEN', 'READY_TO_CLOSE', 'CLOSED'],
    ANSWERED: ['WAITING_FOR_CLIENT', 'CLOSED', 'OPEN', 'IN_PROGRESS'],
    UAT: ['WAITING_FOR_CLIENT', 'CLOSED', 'IN_PROGRESS'],
    RESOLVED: ['READY_TO_CLOSE', 'CLOSED', 'REOPENED', 'OPEN', 'IN_PROGRESS', 'WAITING_FOR_CLIENT'],
    READY_TO_CLOSE: ['WAITING_FOR_CLIENT', 'CLOSED', 'REOPENED', 'OPEN'],
    REOPENED: ['OPEN', 'IN_PROGRESS', 'WAITING_FOR_CLIENT', 'RESOLVED', 'CLOSED'],
    CLOSED: ['REOPENED', 'OPEN', 'IN_PROGRESS', 'WAITING_FOR_CLIENT']
  };
  const roleTransitions = {
    client: {
      ...
      UNDER_REVIEW: ['CLOSED'],
      IN_PROGRESS: ['CLOSED'],
      WAITING_FOR_CLIENT: ['OPEN', 'IN_PROGRESS', 'CLOSED'],
      ANSWERED: ['CLOSED'],
      UAT: ['CLOSED'],
      RESOLVED: ['REOPENED', 'OPEN', 'CLOSED'],
      READY_TO_CLOSE: ['REOPENED', 'OPEN', 'CLOSED'],
    ...
    agent: internalTransitions,
    agent_user: internalTransitions,
    agent_manager: internalTransitions,
    superadmin: {
      NEW: ISSUE_STATUSES.filter((item) => item !== 'NEW'),
      OPEN: ISSUE_STATUSES.filter((item) => item !== 'OPEN'),
      UNDER_REVIEW: ISSUE_STATUSES.filter((item) => item !== 'UNDER_REVIEW'),
      APPROVED: ISSUE_STATUSES.filter((item) => item !== 'APPROVED'),
      IN_PROGRESS: ISSUE_STATUSES.filter((item) => item !== 'IN_PROGRESS'),
      WAITING_FOR_CLIENT: ISSUE_STATUSES.filter((item) => item !== 'WAITING_FOR_CLIENT'),
      ANSWERED: ISSUE_STATUSES.filter((item) => item !== 'ANSWERED'),
      UAT: ISSUE_STATUSES.filter((item) => item !== 'UAT'),
      RESOLVED: ISSUE_STATUSES.filter((item) => item !== 'RESOLVED'),
      READY_TO_CLOSE: ISSUE_STATUSES.filter((item) => item !== 'READY_TO_CLOSE'),
      REOPENED: ISSUE_STATUSES.filter((item) => item !== 'REOPENED'),
      CLOSED: ISSUE_STATUSES.filter((item) => item !== 'CLOSED')
    }
  };
...
async function getWorkflowForIssueType(tenantId, requestType = 'BUG') {
  return WorkflowConfig.findOne({ tenantId, issueType: String(requestType || 'BUG').toUpperCase() }).lean();
}

async function searchAssignableAgents(req, res, next) {
  try {
    if (!canAssignIssue(req.currentUser)) return res.status(403).json({ error: 'You cannot assign issues.' });
    if (['agent','agent_user','agent_manager'].includes(req.currentUser.role) && !(await userHasEntityAccess(req.currentUser, req.issue.entityId._id))) {
      return res.status(403).json({ error: 'You do not have access to assign this issue.' });
    }
    ...
    const items = await User.find({
      ...
      isActive: true,
      role: { $in: ['agent','agent_user','agent_manager','engagement_manager','superadmin'] },
      $or: [{ name: regex }, { email: regex }, { role: regex }]
    }).select('name email role').sort({ name: 1 }).limit(15).lean();
    return res.json({ items: items.map((agent) => ({ _id: String(agent._id), name: agent.name || agent.email || String(agent._id), email: agent.email || '', role: agent.role || '' })) });
  } catch (error) { return next(error); }
}

async function getAllowedStatusTransitionsForIssue({ tenantId, user, currentStatus, requestType = 'BUG' }) {
  const baseAllowed = getAllowedStatusTransitions(user, currentStatus);
  const workflow = await getWorkflowForIssueType(tenantId, requestType);
  if (!baseAllowed.includes('WAITING_FOR_CLIENT') && currentStatus !== 'WAITING_FOR_CLIENT' && ['agent','agent_user','agent_manager','engagement_manager','superadmin'].includes(user.role)) baseAllowed.push('WAITING_FOR_CLIENT');
  if (!workflow || !Array.isArray(workflow.transitions) || !workflow.transitions.length) return baseAllowed;
  const roleAliases = user.role === 'agent_user' ? ['agent_user', 'agent'] : user.role === 'agent_manager' ? ['agent_manager', 'agent'] : [user.role];
  const workflowAllowed = workflow.transitions
    .filter((item) => item.fromStatus === currentStatus && (item.rolesAllowed || []).some((role) => roleAliases.includes(role)))
    .map((item) => item.toStatus)
    .filter((status) => baseAllowed.includes(status));
  const finalAllowed = workflowAllowed.length ? workflowAllowed : baseAllowed;
  if (!finalAllowed.includes('WAITING_FOR_CLIENT') && currentStatus !== 'WAITING_FOR_CLIENT' && ['agent','agent_user','agent_manager','engagement_manager','superadmin'].includes(user.role)) finalAllowed.push('WAITING_FOR_CLIENT');
  return Array.from(new Set(finalAllowed));
}
...
async function buildIssueFilter(req) {
  ...
  if (user.role !== 'superadmin') {
    ...
    filter.entityId = { $in: Array.isArray(allowedEntityIds) && allowedEntityIds.length ? allowedEntityIds : [] };
  }
  ...
  applyEntityMultiFilter(filter, queryValues(incoming.entityId));
  applyMultiFilter(filter, 'status', queryValues(incoming.status));
  applyMultiFilter(filter, 'priority', queryValues(incoming.priority));
  applyMultiFilter(filter, 'category', queryValues(incoming.category));
  applyMultiFilter(filter, 'source', queryValues(incoming.source));
  applyMultiFilter(filter, 'requestType', queryValues(incoming.requestType));
  applyMultiFilter(filter, 'assignedToUserId', queryValues(incoming.assignedToUserId));
  applyMultiFilter(filter, 'createdByUserId', queryValues(incoming.createdByUserId));
  applyMultiFilter(filter, 'triageStatus', queryValues(incoming.triageStatus).filter((item) => TRIAGE_STATUSES.includes(item)));
  applyMultiFilter(filter, 'executionMode', queryValues(incoming.executionMode));
  applyMultiFilter(filter, 'routingStatus', queryValues(incoming.routingStatus));
  applyMultiFilter(filter, 'supportGroupId', queryValues(incoming.supportGroupId));
  applyMultiFilter(filter, 'product', queryValues(incoming.product));

  const customerVisibilityValues = queryValues(incoming.customerVisibility).filter((item) => CUSTOMER_VISIBILITIES.includes(item));
  if (customerVisibilityValues.length && user.role !== 'client') applyMultiFilter(filter, 'customerVisibility', customerVisibilityValues);

  const slaValues = queryValues(incoming.slaStatus);
  if (slaValues.length) {
    filter.$and = [...(filter.$and || []), { $or: [{ 'sla.responseStatus': { $in: slaValues } }, { 'sla.resolutionStatus': { $in: slaValues } }] }];
  }

  const jiraLinked = firstQueryValue(incoming.jiraLinked);
  if (jiraLinked === 'YES') filter['jira.issueKey'] = { $exists: true, $ne: '' };
  if (jiraLinked === 'NO') filter.$and = [...(filter.$and || []), { $or: [{ 'jira.issueKey': { $exists: false } }, { 'jira.issueKey': '' }] }];

  const createdFrom = parseDateOnly(firstQueryValue(incoming.createdFrom));
  const createdTo = parseDateOnly(firstQueryValue(incoming.createdTo), { endOfDay: true });
  ...
  const q = String(firstQueryValue(incoming.q) || '').trim();
  if (q) {
...
async function listIssuesPage(req, res, next) {
  try {
    ...
    const filters = {
      q: firstQueryValue(baseQuery.q),
      entityId: queryValues(baseQuery.entityId),
      status: queryValues(baseQuery.status),
      priority: queryValues(baseQuery.priority),
      category: queryValues(baseQuery.category),
      source: queryValues(baseQuery.source),
      requestType: queryValues(baseQuery.requestType),
      jiraLinked: firstQueryValue(baseQuery.jiraLinked),
      triageStatus: queryValues(baseQuery.triageStatus),
      assignedToUserId: queryValues(baseQuery.assignedToUserId),
      createdByUserId: queryValues(baseQuery.createdByUserId),
      executionMode: queryValues(baseQuery.executionMode),
      routingStatus: queryValues(baseQuery.routingStatus),
      slaStatus: queryValues(baseQuery.slaStatus),
      customerVisibility: queryValues(baseQuery.customerVisibility),
      product: queryValues(baseQuery.product),
      createdFrom: firstQueryValue(baseQuery.createdFrom),
      createdTo: firstQueryValue(baseQuery.createdTo),
      pageSize: 15,
      sortBy: sort.sortBy,
    ...
    if (req.currentUser.role !== 'superadmin') {
      ...
      filter.entityId = { $in: Array.isArray(scopeIds) && scopeIds.length ? scopeIds : [] };
    }
    ...
    applyEntityMultiFilter(filter, filters.entityId);
    applyMultiFilter(filter, 'status', filters.status);
    applyMultiFilter(filter, 'priority', filters.priority);
    applyMultiFilter(filter, 'category', filters.category);
    applyMultiFilter(filter, 'source', filters.source);
    applyMultiFilter(filter, 'requestType', filters.requestType);
    if (filters.jiraLinked === 'YES') filter['jira.issueKey'] = { $exists: true, $ne: '' };
    if (filters.jiraLinked === 'NO') filter.$and = [...(filter.$and || []), { $or: [{ 'jira.issueKey': { $exists: false } }, { 'jira.issueKey': '' }] }];
    applyMultiFilter(filter, 'triageStatus', filters.triageStatus);
    applyMultiFilter(filter, 'assignedToUserId', filters.assignedToUserId);
    applyMultiFilter(filter, 'createdByUserId', filters.createdByUserId);
    applyMultiFilter(filter, 'executionMode', filters.executionMode);
    applyMultiFilter(filter, 'routingStatus', filters.routingStatus);
    if (filters.slaStatus.length) filter.$and = [...(filter.$and || []), { $or: [{ 'sla.responseStatus': { $in: filters.slaStatus } }, { 'sla.resolutionStatus': { $in: filters.slaStatus } }] }];
    if (filters.customerVisibility.length && req.currentUser.role !== 'client') applyMultiFilter(filter, 'customerVisibility', filters.customerVisibility);
    applyMultiFilter(filter, 'product', filters.product);
    ...
    const scopeEntityIds = filter.entityId && filter.entityId.$in ? filter.entityId.$in : null;
    const entityQuery = scopeEntityIds
      ? { tenantId: req.tenant._id, isActive: true, _id: { $in: scopeEntityIds } }
      : { tenantId: req.tenant._id, isActive: true };
    const userQuery = scopeEntityIds
      ? { tenantId: req.tenant._id, isActive: true, role: { $in: ['superadmin', 'agent', 'agent_user', 'agent_manager', 'client', 'engagement_manager'] } }
      : { tenantId: req.tenant._id, isActive: true };

    const [entities, users, savedViews, productOptions] = await Promise.all([
      Entity.find(entityQuery).sort({ path: 1 }).select('_id name path acronym type').lean(),
      User.find(userQuery).sort({ name: 1 }).select('_id name email role').lean(),
      SavedView.find({ tenantId: req.tenant._id, userId: req.currentUser._id }).sort({ isDefault: -1, name: 1 }).lean(),
      Issue.distinct('product', { ...filter, product: { $exists: true, $ne: '' } })
    ]);

    const { page, pageSize, skip } = getPagination({ ...req.query, pageSize: 15 }, 15);
    const totalItems = await Issue.countDocuments(filter);
    const issues = await Issue.find(filter)
      .select('issueNumber title entityId product status jira priority triageStatus assignedToUserId createdByUserId createdAt updatedAt sla closure requestType executionMode routingStatus source customerVisibility tags')
    ...
    return res.render('tickets/list', {
      ...
      priorities: getTenantIssueConfig(req.tenant).priorities,
      severities: getTenantIssueConfig(req.tenant).priorities,
      triageStatuses: TRIAGE_STATUSES,
      categoryOptions: getTenantIssueConfig(req.tenant).categories,
      sourceOptions: getTenantIssueConfig(req.tenant).sources,
      requestTypes: getTenantIssueConfig(req.tenant).requestTypes,
      pager,
      buildQueryString,
      savedViews,
...
function parseCsvText(csvText = '') {
  ...
  const rows = lines.slice(1).map((line, rowIndex) => {
    ...
    headers.forEach((header, columnIndex) => {
      row[header] = values[columnIndex] != null ? values[columnIndex] : '';
    });
...
async function resolveBulkUploadEntity({ tenantId, rawAcronym = '', rawName = '' }) {
  const acronym = String(rawAcronym || '').trim().toUpperCase();
  const name = String(rawName || '').trim();
  if (acronym) {
    const entity = await Entity.findOne({ tenantId, acronym, isActive: true }).lean();
    if (entity) return entity;
    return null;
  }
  if (name) {
    const escaped = escapeRegex(name);
    const exactMatches = await Entity.find({ tenantId, isActive: true, $or: [{ name: new RegExp(`^${escaped}$`, 'i') }, { path: new RegExp(`^${escaped}$`, 'i') }] }).sort({ path: 1 }).lean();
    if (exactMatches.length === 1) return exactMatches[0];
    if (exactMatches.length > 1) {
      const error = new Error(`Multiple entities matched \"${name}\". Use entityAcronym instead.`);
      error.status = 400;
      throw error;
    }
  }
...
function normalizeBulkUploadSource(value = '') {
  const raw = String(value || '').trim().toLowerCase();
  if (!raw) return 'portal';
  return ['portal', 'email', 'api', 'future-ready'].includes(raw) ? raw : null;
}

async function showBulkIssueUploadPage(req, res, next) {
  try {
    if (!['superadmin', 'agent'].includes(req.currentUser.role)) throw forbidden('Only agents or superadmins can bulk upload issues.');
    const entities = await listCreatableEntitiesForUser(req.currentUser);
    const masters = getTenantIssueConfig(req.tenant);
    return res.render('tickets/bulk-upload', {
      title: 'Bulk Upload Issues',
      entities,
      priorities: masters.priorities,
      severities: masters.priorities,
      categoryOptions: masters.categories,
      sourceOptions: masters.sources,
      requestTypes: masters.requestTypes,
      requestTaxonomy: masters.requestTaxonomy,
      result: null
    });
  } catch (error) {
    return next(error);
...
function downloadBulkIssueTemplate(req, res) {
  const headers = ['entityAcronym', 'title', 'description', 'requestType', 'priority', 'category', 'source', 'tags', 'customerVisibility'];
  const rows = [[
    'C001',
    'Login failure in customer portal',
  ...
  return sendCsv(res, `issues-upload-template-${req.tenant.slug}.csv`, headers, rows);
}

async function bulkUploadIssues(req, res, next) {
  try {
    if (!['superadmin', 'agent'].includes(req.currentUser.role)) throw forbidden('Only agents or superadmins can bulk upload issues.');
    const entities = await listCreatableEntitiesForUser(req.currentUser);
    const masters = getTenantIssueConfig(req.tenant);
    const categoryOptions = masters.categories;
    const sourceOptions = masters.sources;
    const priorityOptions = masters.priorities;

    const file = (req.files || [])[0];
    if (!file) throw badRequest('Please upload a CSV file.');
    const csvText = file.buffer.toString('utf8');
    const parsed = parseCsvText(csvText);
    if (!parsed.headers.length) throw badRequest('The CSV file is empty.');

    const supportedHeaders = ['entityAcronym', 'entityCode', 'entityName', 'title', 'description', 'requestType', 'priority', 'category', 'source', 'tags', 'customerVisibility'];
    const hasEntityColumn = parsed.headers.some((header) => ['entityAcronym', 'entityCode', 'entityName'].includes(header));
    if (!hasEntityColumn) throw badRequest('The template must include entityAcronym, entityCode, or entityName.');
    ...
    for (const row of parsed.rows) {
      try {
        const entityLookupValue = row.entityAcronym || row.entityCode || '';
        const entity = await resolveBulkUploadEntity({ tenantId: req.tenant._id, rawAcronym: entityLookupValue, rawName: row.entityName || '' });
        if (!entity) {
          if (entityLookupValue) throw badRequest(`Entity acronym ${String(entityLookupValue).trim().toUpperCase()} not found.`);
          throw badRequest('entityAcronym or entityName is required.');
        }
        if (!(await userHasEntityAccess(req.currentUser, entity._id))) throw forbidden(`You do not have access to entity ${entity.path || entity.name}.`);

        const title = String(row.title || '').trim();
        const description = String(row.description || '').trim();
        const requestType = resolveRequestType(row.requestType || 'BUG', 'BUG', req.tenant);
        const priority = resolveConfiguredPriority(row.priority || priorityOptions[1] || priorityOptions[0], req.tenant, priorityOptions[0]);
        const category = resolveRequestClassification({ tenant: req.tenant, requestType, category: row.category || categoryOptions[0] });
        const source = resolveConfiguredSource(row.source || 'bulk_upload', req.tenant, 'bulk_upload');
        const visibility = String(row.customerVisibility || 'VISIBLE_TO_CUSTOMER').trim().toUpperCase();
        const tags = String(row.tags || '').split(',').map((item) => String(item || '').trim()).filter(Boolean);

        if (!title) throw badRequest('Title is required.');
        if (!description) throw badRequest('Description is required.');
        if (!requestType) throw badRequest(`Request type ${row.requestType} is invalid.`);
        if (!priority) throw badRequest(`Priority ${row.priority} is invalid.`);
        if (!category) throw badRequest(`Category ${row.category} is invalid.`);
        if (!source) throw badRequest(`Source ${row.source} is invalid.`);
        if (!CUSTOMER_VISIBILITIES.includes(visibility)) throw badRequest(`customerVisibility ${visibility} is invalid.`);

        const issueNumber = await generateIssueNumber({ tenantId: req.tenant._id, entity });
        const routing = await resolveRouting({ tenantId: req.tenant._id, entityId: entity._id, category, priority });
        const stageStatusSeed = [{ status: 'NEW', fromStatus: '', enteredAt: new Date(), exitedAt: null, durationMinutes: 0, excludeFromFinalSla: false }];
        ...
        const issue = await Issue.create({
          ...
          source,
          status: 'NEW',
          routingStatus: routing.routingRuleId ? 'ROUTED' : 'NO_MATCH',
      ...
      assignedByUserId: null,
        ...
        await createIssueActivity({
          ...
          type: 'ISSUE_CREATED',
          metadata: { via: 'BULK_UPLOAD', entityAcronym: entity.acronym, requestType },
          performedByUserId: req.currentUser._id,
          performedByRole: getReporterTypeForUser(req.currentUser)
        });
        await logAudit({ tenantId: req.tenant._id, actorUserId: req.currentUser._id, action: 'issue.bulk_created', entityType: 'issue', entityId: issue._id, after: { issueNumber: issue.issueNumber, entityAcronym: entity.acronym, via: 'bulk_upload' } });
        createdIssues.push({ issueNumber: issue.issueNumber, entity: entity.path || entity.name });
      } catch (rowError) {
        rowErrors.push(`Row ${row.__rowNumber}: ${rowError.message}`);
      }
    }

    return res.status(rowErrors.length ? 400 : 200).render('tickets/bulk-upload', {
      title: 'Bulk Upload Issues',
      entities,
      priorities: priorityOptions,
      severities: priorityOptions,
      categoryOptions,
      sourceOptions,
      requestTypes: masters.requestTypes,
      requestTaxonomy: masters.requestTaxonomy,
  ...
  try {
    ...
    const selectedEntityId = requestedEntityId && entities.some((entity) => String(entity._id) === String(requestedEntityId))
      ...
      : entities.length === 1
        ...
        : '';
    const masters = getTenantIssueConfig(req.tenant);
    const modules = await ModuleDefinition.find({ tenantId: req.tenant._id, isActive: true }).sort({ name: 1 }).lean();
    const entityTaxonomyMap = {};
    entities.forEach((entity) => { entityTaxonomyMap[String(entity._id)] = mergeEntityRequestTaxonomy(entity, req.tenant); });
    ...
    return res.render('tickets/new', {
      ...
      entityTaxonomyMap,
      modules,
      defaults: { priority: masters.priorities[1] || masters.priorities[0] || 'MEDIUM', severity: masters.priorities[1] || masters.priorities[0] || 'MEDIUM', source: masters.sources[0] || 'portal', entityId: selectedEntityId, category: masters.categories[0] || 'General', requestType: 'BUG', customerVisibility: 'VISIBLE_TO_CUSTOMER' },
...
async function createIssue(req, res, next) {
  try {
    const { entityId, title, description, priority = '', severity = '', category = '', requestType = 'BUG', tags = '', source = 'portal', customerVisibility: requestedCustomerVisibility = 'VISIBLE_TO_CUSTOMER', affectedUsers = '', businessImpact = '', isBlocking = '', workaroundAvailable = '', environment = '', startedAtText = '', extraContext = '', module = '', stepsToReproduce = '', expectedBehavior = '', actualBehavior = '', businessJustification = '', expectedOutcome = '', requestedTimeline = '', impactedModules = '', clarificationNeeded = '', referenceContext = '', queryUrgency = '', requiredByDate = '', serviceReference = '', serviceRequestDetails = '', serviceEnvironment = '', serviceRequestKind = '' } = req.body;
    const selectedModules = Array.isArray(req.body.modules) ? req.body.modules : (req.body.modules ? [req.body.modules] : (module ? [module] : []));
    ...
    const masters = getTenantIssueConfig(req.tenant);
    if (!canCreateIssue(req.currentUser.role)) throw forbidden('This role can view issues but cannot create them.');
    ...
    const resolvedPriority = resolveConfiguredPriority(severity || priority, req.tenant, masters.priorities[0]);
    const resolvedRequestType = resolveRequestTypeForEntity(requestType, 'BUG', req.tenant, entity);
    const resolvedCategory = resolveRequestClassificationForEntity({ tenant: req.tenant, entity, requestType: resolvedRequestType || 'BUG', category });
    const resolvedSource = req.currentUser.role === 'client' ? 'portal' : resolveConfiguredSource(source, req.tenant, masters.sources[0] || 'portal');
    if (!resolvedPriority) throw badRequest('Priority is invalid.');
    if (!resolvedCategory) throw badRequest('Category/classification is required for the selected request type.');
    if (!resolvedSource) throw badRequest('Source is invalid.');
    if (!resolvedRequestType) throw badRequest('Request type is invalid for the selected entity.');

    const [routing, resolvedJira] = await Promise.all([
      resolveRouting({
        tenantId: req.tenant._id,
        entityId,
      ...
      resolveEffectiveEntityJiraConfig({ tenantId: req.tenant._id, entityId })
    ]);
    const routingDecision = {
      ...
      trace: [{ step: 'category', value: resolvedCategory }, { step: 'priority', value: resolvedPriority }, { step: 'entityId', value: String(entityId) }, { step: 'supportGroupId', value: routing.supportGroupId ? String(routing.supportGroupId) : '' }, { step: 'executionMode', value: routing.executionMode || 'NATIVE' }, { step: 'jiraProjectKey', value: routing.jiraProjectKey || '' }]
    };

    const connection = resolvedJira.config ? await getTenantJiraConnection({ tenantId: req.tenant._id, includeSecret: true }) : null;
    const metadataResult = resolvedJira.config ? await getJiraMetadataForIssueContext({ tenantId: req.tenant._id, entityId, resolvedConfig: resolvedJira.config, connection }) : { fields: [], source: 'NONE' };
    const jiraFieldValues = parseJiraFieldInputs(req.body);
    ...
    const slaSnapshot = buildSlaSnapshot({ policy: resolvedSlaPolicy, startedAt: new Date(), severity: resolvedPriority });
    const commitmentSnapshots = buildCommitmentSnapshots(agreementBundle.bundle, new Date(), resolvedPriority);
    ...
    const issue = await Issue.create({
      ...
      priority: resolvedPriority,
      category: resolvedCategory,
      requestClassification: resolvedCategory,
      product: entity.metadata?.product || '',
      module: selectedModules.map((item) => String(item || '').trim()).filter(Boolean).join(', '),
      createdByUserId: req.currentUser._id,
      lastUpdatedByUserId: req.currentUser._id,
      assignedToUserId: null,
      assignmentMode: 'UNASSIGNED',
      ...
      triageStatus: 'NOT_TRIAGED',
      intake: {
        affectedUsers: String(affectedUsers || '').trim(),
        businessImpact: String(businessImpact || '').trim(),
        isBlocking: ['YES','NO','PARTIAL'].includes(String(isBlocking || '').toUpperCase()) ? String(isBlocking).toUpperCase() : '',
        workaroundAvailable: ['YES','NO','UNKNOWN'].includes(String(workaroundAvailable || '').toUpperCase()) ? String(workaroundAvailable).toUpperCase() : '',
        environment: String(environment || '').trim(),
        startedAtText: String(startedAtText || '').trim(),
        extraContext: String(extraContext || '').trim(),
        typeSpecific: {
          issue: { stepsToReproduce: String(stepsToReproduce || '').trim(), expectedBehavior: String(expectedBehavior || '').trim(), actualBehavior: String(actualBehavior || '').trim() },
          changeRequest: { businessJustification: String(businessJustification || '').trim(), expectedOutcome: String(expectedOutcome || '').trim(), requestedTimeline: String(requestedTimeline || '').trim(), impactedModules: String(impactedModules || '').trim() },
          query: { clarificationNeeded: String(clarificationNeeded || '').trim(), referenceContext: String(referenceContext || '').trim(), queryUrgency: String(queryUrgency || '').trim() },
          serviceRequest: { serviceRequestKind: String(serviceRequestKind || '').trim(), requiredByDate: String(requiredByDate || '').trim(), serviceEnvironment: String(serviceEnvironment || '').trim(), serviceReference: String(serviceReference || '').trim(), serviceRequestDetails: String(serviceRequestDetails || '').trim() }
        }
      },
      ai: {
        classification: {
          suggestedSeverity: resolvedPriority,
          suggestedCategory: resolvedCategory,
          suggestedRequestType: resolvedRequestType,
          suggestedClassification: resolvedCategory,
          rationale: 'Initial deterministic v31 classification from guided intake fields.'
        },
        confidence: 0.65,
      ...
      jiraDraft: resolvedJira.config ? {
        ...
        issueTypeName: getJiraIssueTypeForRequestType(req.tenant, resolvedRequestType) || resolvedJira.config.issueTypeName || '',
      ...
      sla: { ...slaSnapshot, stageStatus: stageStatusSeed },
      ...
      source: customerVisibility === 'INTERNAL_ONLY' ? 'internal' : resolvedSource,
      customerVisibility
    });
...
if (resolvedJira.config?.isEnabled && resolvedJira.config.autoPushOnCreate) {
  ...
  if (missingRequiredFields.length) {
    ...
    await issue.save();
    deferWork('issue auto-push failed activity', () => createIssueActivity({ tenantId: req.tenant._id, issueId: issue._id, entityId: issue.entityId, type: 'ISSUE_AUTO_PUSH_FAILED', metadata: { reason: autoPushErrorMessage }, performedByUserId: req.currentUser._id, performedByRole: getReporterTypeForUser(req.currentUser) }));
    ...
    deferWork('issue auto-push queued side effects', async () => {
    ...
    deferWork('issue create side effects', async () => {
    ...
    const responseIssue = Object.assign(issue.toObject(), { tenantSlug: req.tenant.slug });
    if (routing.matchedRule?.supportGroupId) {
      responseIssue.supportGroupId = routing.matchedRule.supportGroupId;
    }
    if (routing.assignedToUserId) {
      responseIssue.assignedToUserId = routing.assignedToUserId;
    }

    if (isApiRequest(req)) return res.status(201).json({ item: issueToJson(responseIssue) });
...
async function viewIssuePage(req, res, next) {
  try {
    ...
    const [agents, comments, historyItems, jiraLink, auditTrail, jiraConnection] = await Promise.all([
      User.find({ tenantId: req.tenant._id, isActive: true, role: { $in: ['agent','agent_user','agent_manager','engagement_manager','superadmin'] } }).select('name email role').sort({ name: 1 }).limit(40).lean(),
    ...
    return res.render('tickets/detail', {
      ...
      sourceOptions: getTenantIssueConfig(req.tenant).sources,
      requestTypes: getTenantIssueConfig(req.tenant).requestTypes,
      allowedStatusTransitions: await getAllowedStatusTransitionsForIssue({ tenantId: req.tenant._id, user: req.currentUser, currentStatus: issue.status, requestType: issue.requestType || 'BUG' }),
      comments,
      historyItems,
...
async function searchSimilarIssues(req, res, next) {
  try {
    const q = String(req.query.q || '').trim();
    if (q.length < 4) return res.json({ items: [] });
    const tokens = q.toLowerCase().split(/[^a-z0-9]+/i).filter((w) => w.length >= 4).slice(0, 8);
    if (!tokens.length) return res.json({ items: [] });
    const regexes = tokens.map((token) => new RegExp(token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'));
    const filter = await buildIssueFilter({ ...req, query: {} });
    const items = await Issue.find({ ...filter, $or: regexes.flatMap((r) => [{ title: r }, { description: r }, { tags: r }, { issueNumber: r }]) })
      .select('issueNumber title status priority entityId createdAt updatedAt')
      .populate('entityId', 'name path')
      .sort({ updatedAt: -1 })
      .limit(8)
      .lean();
    return res.json({ items: items.map((issue) => ({ id: String(issue._id), issueNumber: issue.issueNumber, title: issue.title, status: issue.status, priority: issue.priority, entity: issue.entityId?.path || issue.entityId?.name || '', url: `${req.basePath}/tickets/${issue._id}` })) });
  } catch (error) { return next(error); }
}
...
async function listIssuesApi(req, res, next) {
  try {
    ...
    const items = await Issue.find(filter)
      .select('issueNumber title entityId product status jira priority triageStatus assignedToUserId createdByUserId triagedByUserId createdAt updatedAt sla closure requestType executionMode routingStatus source customerVisibility tags')
      .populate('entityId', 'name path acronym type')
      .populate('createdByUserId', 'name email role')
      .populate('assignedToUserId', 'name email role')
      .populate('triagedByUserId', 'name email role')
      .sort({ updatedAt: -1 })
      .lean();
...
async function updateIssueStatus(req, res, next) {
  try {
    ...
    const { status, rootCause = '', resolutionSummary = '', preventiveAction = '', customerClosureNote = '' } = req.body;
    ...
    const allowedTransitions = await getAllowedStatusTransitionsForIssue({ tenantId: req.tenant._id, user: req.currentUser, currentStatus: beforeStatus, requestType: req.issue.requestType || 'BUG' });
    const workflow = await getWorkflowForIssueType(req.tenant._id, req.issue.requestType || 'BUG');
    const workflowRule = workflow?.transitions?.find((item) => item.fromStatus === beforeStatus && item.toStatus === status);
    const currentRoleAliases = req.currentUser.role === 'agent_user' ? ['agent_user', 'agent'] : req.currentUser.role === 'agent_manager' ? ['agent_manager', 'agent'] : [req.currentUser.role];
    if (workflowRule && !(req.currentUser.role === 'client' && status === 'CLOSED') && !(workflowRule.rolesAllowed || []).some((role) => currentRoleAliases.includes(role))) {
    ...
    if (status === 'CLOSED' && req.currentUser.role !== 'client' && req.issue.executionMode === 'JIRA' && !req.issue.closure?.awaitingAgentClosure && req.issue.jira?.issueKey) {
    ...
    if (status === 'CLOSED') {
      const isClientClosure = req.currentUser.role === 'client';
      if (!isClientClosure && (!String(rootCause).trim() || !String(resolutionSummary).trim())) {
        const message = 'Root cause and resolution summary are required before closing this issue.';
        if (isApiRequest(req)) return res.status(400).json({ error: message });
        req.session.error = message;
        return res.redirect(`${req.basePath}/tickets/${req.issue._id}`);
      }
      if (isClientClosure && !String(customerClosureNote || resolutionSummary || rootCause).trim()) {
        const message = 'Please add a short closure note before closing this issue.';
        if (isApiRequest(req)) return res.status(400).json({ error: message });
        req.session.error = message;
        return res.redirect(`${req.basePath}/tickets/${req.issue._id}`);
      }
    ...
    pushStageTracker(req.issue, beforeStatus, status);
    req.issue.status = status;
    if (status === 'WAITING_FOR_CLIENT' && req.issue.sla && !req.issue.sla.firstRespondedAt) { req.issue.sla.firstRespondedAt = new Date(); req.issue.sla.respondedByUserId = req.currentUser._id; }
    if (status === 'WAITING_FOR_CLIENT') { pauseIssueSla(req.issue); appendSlaEvent(req.issue, 'SLA_PAUSED', { reason: 'WAITING_FOR_CLIENT' }); } else { resumeIssueSla(req.issue); if (beforeStatus === 'WAITING_FOR_CLIENT') appendSlaEvent(req.issue, 'SLA_RESUMED', { reason: status }); }
    if (status === 'CLOSED') {
      ...
      req.issue.closure.closureSource = req.currentUser.role === 'client' ? 'CLIENT_DIRECT' : (req.currentUser.role === 'superadmin' ? 'SUPERADMIN' : 'AGENT');
      req.issue.closure.rca = req.issue.closure.rca || {};
      req.issue.closure.rca.rootCause = String(rootCause || '').trim();
      req.issue.closure.rca.resolutionSummary = String(resolutionSummary || '').trim();
      req.issue.closure.rca.preventiveAction = String(preventiveAction || '').trim();
      req.issue.closure.rca.customerClosureNote = String(customerClosureNote || '').trim();
      req.issue.closure.rca.enteredByUserId = req.currentUser._id;
      req.issue.closure.rca.enteredAt = new Date();
    }
    ...
    if (status === 'CLOSED' && req.currentUser.role !== 'client') {
      await createKbDraftFromClosedIssue({ tenantId: req.tenant._id, issue: req.issue, userId: req.currentUser._id });
    }

    const reopened = ['RESOLVED', 'READY_TO_CLOSE', 'CLOSED'].includes(beforeStatus) && ['REOPENED','OPEN','IN_PROGRESS'].includes(status);
    await createIssueActivity({
      tenantId: req.tenant._id,
      issueId: req.issue._id,
      entityId: req.issue.entityId._id,
    ...
    const detailBase = req.originalUrl.includes('/client/') ? `${req.basePath}/client/issues` : `${req.basePath}/tickets`;
    return res.redirect(`${detailBase}/${req.issue._id}`);
  } catch (error) {
    return next(error);
  }
}
...
async function assignIssue(req, res, next) {
  try {
    ...
    try {
      ...
      // v35 pragmatic governance: allow managers to assign active internal tenant users even before formal entity membership is created.
      assignee = await User.findOne({ _id: assignedToUserId, tenantId: req.tenant._id, isActive: true, role: { $in: ['agent','agent_user','agent_manager','engagement_manager','superadmin'] } });
      if (!assignee) throw validationError;
    }
    ...
    req.issue.assignmentMode = 'PUSH';
    req.issue.assignedByUserId = req.currentUser._id;
    req.issue.lastUpdatedByUserId = req.currentUser._id;
    await req.issue.save();
...
async function takeOwnership(req, res, next) {
  try {
    if (!canTakeOwnership(req.currentUser.role)) {
      const message = 'Only agent users, agent managers, or superadmins can take ownership.';
      if (isApiRequest(req)) return res.status(403).json({ error: message });
      req.session.error = message;
      return res.redirect(`${req.basePath}/tickets/${req.params.id}`);
    }
    if (!(await userHasEntityAccess(req.currentUser, req.issue.entityId._id))) {
      const message = 'You do not have access to take this issue.';
      if (isApiRequest(req)) return res.status(403).json({ error: message });
      req.session.error = message;
      return res.redirect(`${req.basePath}/tickets/${req.params.id}`);
    }
    const beforeAssignedToUserId = normalizeId(req.issue.assignedToUserId);
    const beforeName = req.issue.assignedToUserId ? (req.issue.assignedToUserId.name || req.issue.assignedToUserId.email || String(req.issue.assignedToUserId._id || req.issue.assignedToUserId)) : 'Unassigned';
    req.issue.assignedToUserId = req.currentUser._id;
    req.issue.assignmentMode = 'PULL';
    req.issue.assignedByUserId = req.currentUser._id;
    req.issue.lastUpdatedByUserId = req.currentUser._id;
    await req.issue.save();
    await createIssueActivity({ tenantId: req.tenant._id, issueId: req.issue._id, entityId: req.issue.entityId._id, type: 'ASSIGNED', metadata: { before: beforeName, after: req.currentUser.name || req.currentUser.email, beforeAssignedToUserId, afterAssignedToUserId: req.currentUser._id.toString(), mode: 'pull_take_ownership' }, performedByUserId: req.currentUser._id, performedByRole: getReporterTypeForUser(req.currentUser) });
    await logAudit({ tenantId: req.tenant._id, actorUserId: req.currentUser._id, action: 'issue.taken', entityType: 'issue', entityId: req.issue._id, before: { assignedToUserId: beforeAssignedToUserId }, after: { assignedToUserId: req.currentUser._id.toString() } });
    req.session.success = 'Issue taken successfully.';
    return res.redirect(`${req.basePath}/tickets/${req.issue._id}`);
  } catch (error) { return next(error); }
}
...
async function triageIssue(req, res, next) {
  try {
    ...
    const before = {
      ...
      requestType: req.issue.requestType || 'BUG'
    };

    const category = typeof req.body.category === 'string' && req.body.category.trim() ? req.body.category.trim() : req.issue.category;
    const priority = typeof (req.body.severity || req.body.priority) === 'string' && String(req.body.severity || req.body.priority).trim()
      ? String(req.body.severity || req.body.priority).trim()
      : req.issue.priority;
    const requestType = typeof req.body.requestType === 'string' && req.body.requestType.trim() ? req.body.requestType.trim() : (req.issue.requestType || 'BUG');
    ...
    const resolvedCategory = resolveConfiguredCategory(category, req.tenant, req.issue.category);
    const resolvedPriority = resolveConfiguredPriority(priority, req.tenant, req.issue.priority);
    const resolvedRequestType = resolveRequestType(requestType, req.issue.requestType || 'BUG');
    if (!resolvedCategory) throw badRequest('category is invalid for triage.');
    if (!resolvedPriority) throw badRequest('priority is invalid.');
    if (!resolvedRequestType) throw badRequest('request type is invalid.');
    ...
    req.issue.category = resolvedCategory;
    req.issue.priority = resolvedPriority;
    req.issue.requestType = resolvedRequestType;
    ...
    if (nextStatus) {
      const allowedTransitions = await getAllowedStatusTransitionsForIssue({ tenantId: req.tenant._id, user: req.currentUser, currentStatus: req.issue.status, requestType: req.issue.requestType || resolvedRequestType || 'BUG' });
      const workflow = await getWorkflowForIssueType(req.tenant._id, req.issue.requestType || resolvedRequestType || 'BUG');
      ...
      pushStageTracker(req.issue, req.issue.status, nextStatus);
...
async function syncIssueFromJira(req, res, next) {
  try {
    if (!['agent', 'agent_user', 'agent_manager', 'superadmin'].includes(req.currentUser.role)) {
      const message = 'Only internal users can refresh Jira status.';
      if (isApiRequest(req)) return res.status(403).json({ error: message });
      req.session.error = message;
      return res.redirect(`${req.basePath}/tickets/${req.params.id}`);
    }
...
async function pushIssueToJira(req, res, next) {
  try {
    if (!['agent', 'agent_user', 'agent_manager', 'superadmin'].includes(req.currentUser.role)) {
      const message = 'Only internal users can push issues to Jira.';
      if (isApiRequest(req)) return res.status(403).json({ error: message });
      req.session.error = message;
      return res.redirect(`${req.basePath}/tickets/${req.params.id}`);
    }
    ...
    const selectedRequestType = resolveRequestType(req.body?.requestType || req.issue.requestType || 'BUG', req.issue.requestType || 'BUG');
    if (!selectedRequestType) {
      const message = 'Select whether this ticket is a Bug or a CR before pushing to Jira.';
      if (isApiRequest(req)) return res.status(400).json({ error: message });
      req.session.error = message;
      return res.redirect(`${req.basePath}/tickets/${req.params.id}`);
    }
    req.issue.requestType = selectedRequestType;
    const requestedIssueTypeName = String(req.body?.issueTypeName || getJiraIssueTypeForRequestType(req.tenant, selectedRequestType)).trim();
...
async function createComment(req, res, next) {
  try {
    ...
    deferWork('comment create side effects', async () => {
      await createIssueActivity({
        tenantId: req.tenant._id,
        issueId: req.issue._id,
        entityId: req.issue.entityId._id,
    ...
    if (isApiRequest(req)) return res.status(201).json({ item: commentToJson(Object.assign(comment.toObject(), { tenantSlug: req.tenant.slug, authorUserId: req.currentUser })) });
...
async function bulkUpdateIssues(req, res, next) {
  try {
    if (!['agent', 'agent_user', 'agent_manager', 'superadmin'].includes(req.currentUser.role)) return res.status(403).json({ error: 'Only agents or superadmins can bulk update issues.' });
...
module.exports = {
  listIssuesPage,
  showBulkIssueUploadPage,
  downloadBulkIssueTemplate,
  bulkUploadIssues,
  showCreateIssue,
  createIssue,
  getIssueOrDeny,
