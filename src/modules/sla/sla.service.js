...
function buildSlaSnapshot({ policy, startedAt = new Date(), severity = null }) {
  if (!policy) {
    return {
      ...
      permanentFixTargetMinutes: null,
      permanentFixDueAt: null,
      permanentFixStatus: 'NO_SLA',
  ...
  const permanentFixTargetMinutes = getMetricMinutes(metricTargets, 'PERMANENT_FIX', null);
  ...
  const permanentFixDueAt = permanentFixTargetMinutes != null ? addMinutes(new Date(startedAt), permanentFixTargetMinutes) : null;
  return {
    hasPolicy: true,
    policyId: policy._id,
    ...
    permanentFixTargetMinutes,
    permanentFixDueAt,
    permanentFixStatus: getIndicator({ dueAt: permanentFixDueAt, warningThresholdPercent: policy.warningThresholdPercent || 80, startedAt }),
...
function evaluateIssueSla(issue) {
  ...
  if (sla.pausedAt) {
    ...
    sla.resolutionStatus = sla.resolvedAt ? getIndicator({ dueAt: sla.resolutionDueAt, completedAt: sla.resolvedAt, warningThresholdPercent: sla.warningThresholdPercent, startedAt: issue.createdAt || new Date() }) : (sla.resolutionStatus || 'ON_TRACK');
