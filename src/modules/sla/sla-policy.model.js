...
const MATCH_ANY = 'ANY';
const PRIORITY_OPTIONS = ['ANY', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'BLOCKER']; // fallback/default master list; tenant-configured priorities may extend or replace these
...
const METRIC_TYPES = ['FIRST_RESPONSE', 'ACKNOWLEDGEMENT', 'WORKAROUND', 'RESOLUTION', 'PERMANENT_FIX', 'UPDATE_FREQUENCY', 'CLOSURE_CONFIRMATION'];
...
const slaPolicySchema = new mongoose.Schema({
  ...
  supportPackage: { type: String, default: 'Standard', trim: true, index: true },
  ...
  priority: { type: String, default: MATCH_ANY, trim: true, index: true },
