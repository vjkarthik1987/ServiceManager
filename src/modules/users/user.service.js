...
const { listApprovedUsersForTenant, linkApprovedUserToUser } = require('./approved-user.service');
...
function generateTemporaryPassword(length = 12) {
  const safeLength = Number.isFinite(Number(length)) ? Math.max(10, Number(length)) : 12;
...
async function assertApprovedPoolEligibility({ tenantId, name, email, role }) {
  if (!['agent', 'agent_user', 'agent_manager'].includes(role)) return null;
  const approvedUsers = await listApprovedUsersForTenant({ tenantId, onlyActive: true });
  const cleanName = sanitizeString(name);
  const cleanEmail = normalizeEmail(email);
  const match = approvedUsers.find((item) => item.email === cleanEmail && sanitizeString(item.name) === cleanName);
  if (!match) {
    const err = new Error('Agent users and agent managers can be created only from the approved user pool.');
    err.status = 400;
    throw err;
  }
  return match;
...
function validateRole(role) {
  if (!['superadmin', 'agent', 'agent_manager', 'agent_user', 'client', 'engagement_manager', 'regional_head', 'support_head'].includes(role)) {
    const err = new Error('Role must be superadmin, agent manager, agent user, engagement manager, agent, or client.');
    err.status = 400;
    throw err;
  }
}

async function createUserForTenant({ tenantId, tenant = null, name, email, password, role, entityId, entityIds = [], regions = [], createdByUser = null, sendProvisioningEmail = true }) {
  const cleanName = sanitizeString(name);
  const cleanEmail = normalizeEmail(email);
  ...
  validateRole(role);
  const approvedPoolMatch = await assertApprovedPoolEligibility({ tenantId, name: cleanName, email: cleanEmail, role });
  ...
  const user = await User.create({
    ...
    role,
    isActive: true,
    regions: Array.isArray(regions) ? regions.map((item) => sanitizeString(item)).filter(Boolean) : []
  });

  await createMembershipsForNewUser({ tenantId, user, role, entityId, entityIds });
  if (approvedPoolMatch) {
    await linkApprovedUserToUser({ tenantId, email: cleanEmail, userId: user._id });
  }
...
async function listUsersForTenant(tenantId, role = null) {
  ...
  const memberships = await UserEntityMembership.find({ tenantId, userId: { $in: userIds }, status: 'active' })
  ...
  memberships.forEach((membership) => {
    ...
    byUser.get(key).push(membership);
  });
  return users.map((user) => ({ user, memberships: byUser.get(String(user._id)) || [] }));
}
...
module.exports = { createUserForTenant, listUsersForTenant, updateUserForTenant, changePasswordForUser, requestPasswordResetForTenant, resetPasswordWithTokenForTenant, generateTemporaryPassword }; 
