...
async function createMembershipsForNewUser({ tenantId, user, role, entityId, entityIds = [] }) {
  ...
  if (role === 'regional_head') {
    let normalizedIds = Array.from(new Set([...(Array.isArray(entityIds) ? entityIds : []), ...(entityId ? [entityId] : [])].filter(Boolean).map(String)));
    if (!normalizedIds.length && Array.isArray(user.regions) && user.regions.length) {
      const regionNames = user.regions.map((item) => String(item || '').trim()).filter(Boolean);
      const regionalEntities = await Entity.find({ tenantId, isActive: true, 'metadata.region': { $in: regionNames } }).select('_id').lean();
      normalizedIds = regionalEntities.map((item) => String(item._id));
    }
    const memberships = [];
    for (const id of normalizedIds) {
      const entity = await validateEntityInTenant({ tenantId, entityId: id });
  ...
  if (['agent', 'agent_user', 'agent_manager', 'engagement_manager', 'support_head'].includes(role)) {
    const normalizedIds = Array.from(
      new Set(
        [
          ...(Array.isArray(entityIds) ? entityIds : []),
...
async function assignAgentToEntity({ tenantId, agentUserId, entityId }) {
  ...
  if (!['agent', 'agent_user', 'agent_manager'].includes(agent.role)) {
    const err = new Error('Only agent managers or agent users can be assigned to entities.');
    err.status = 400;
    throw err;
  }
