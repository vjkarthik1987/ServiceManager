...
const { markNotificationRead, sendBrandedTestEmail } = require('./notification.service');
...
router.post('/test-email', async (req, res, next) => {
  try {
    const recipientEmail = String(req.body.recipientEmail || req.currentUser.email || '').trim();
    const result = await sendBrandedTestEmail({ tenant: req.tenant, recipientEmail, actorUser: req.currentUser });
    if (result.delivered) {
      req.session.success = `Test email sent to ${result.recipient}.`;
    } else {
      req.session.error = `Test email failed: ${result.reason || 'unknown error'}.`;
    }
