    ...
    upload.array(fieldName, MAX_FILE_COUNT)(req, res, (error) => {
      ...
      try {
        for (const file of req.files || []) {
          ...
          const zipContainerAllowed = zipContainerExtensions.has(extension) && (
          ...
          // Browser/proxy MIME reporting is inconsistent, especially for CSV, EML,
          // Office files and generated exports. Once the extension and declared MIME
          // have passed the allow-list above, keep signature detection as metadata
          // instead of failing valid business attachments. Strict mismatch blocking can
          // be re-enabled with ATTACHMENT_STRICT_SIGNATURE_VALIDATION=true.
          const strictSignatureValidation = String(process.env.ATTACHMENT_STRICT_SIGNATURE_VALIDATION || 'false') === 'true';
          if (strictSignatureValidation && !detectedAllowed && !zipContainerAllowed && !textLikeAllowed && !octetStreamAllowed) {
