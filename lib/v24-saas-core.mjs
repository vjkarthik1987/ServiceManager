export const V24_SAAS_SERVICE_MODEL_KEY = 'SUNTEC_SAAS_V24';
export const V24_MARKER_FIELD_KEY = '__v24_service_model_key';
...
export function getServiceModelKey(value = {}) {
  const direct = text(value.serviceModelKey || value.serviceModel?.key || value.v24ServiceModelKey || value.v23ServiceModelKey);
  ...
  const marker = fields.find((field) => normalized(customFieldKey(field)) === normalized(V24_MARKER_FIELD_KEY)
    || normalized(customFieldKey(field)) === 'service model key');
  return marker ? customFieldValue(marker) : '';
}

export function isV24SaasRequest(value = {}) {
  return getServiceModelKey(value) === V24_SAAS_SERVICE_MODEL_KEY;
}
...
export function v24Family(value = {}) {
  if (!isV24SaasRequest(value)) return '';
...
export function isV24SaasIncident(value = {}) {
  return isV24SaasRequest(value) && v24Family(value) === 'INCIDENT';
}

export function isV24SaasServiceRequest(value = {}) {
  return isV24SaasRequest(value) && v24Family(value) === 'SERVICE_REQUEST';
}

export function requestFamilyDisablesSla(value = {}) {
  if (!isV24SaasRequest(value)) return false;
  return isV24SaasServiceRequest(value) || v24Family(value) === 'QUERY';
}

export function automaticWorkflowTasksEnabled(value = {}) {
  return !isV24SaasIncident(value);
}

export function ensureMarkerCustomField(fields = [], extra = {}) {
  const list = Array.isArray(fields) ? fields.map((field) => ({ ...field })) : [];
  const idx = list.findIndex((field) => normalized(customFieldKey(field)) === normalized(V24_MARKER_FIELD_KEY)
    || normalized(customFieldKey(field)) === 'service model key');
  const marker = {
    fieldKey: V24_MARKER_FIELD_KEY,
    key: V24_MARKER_FIELD_KEY,
    code: V24_MARKER_FIELD_KEY,
    name: 'Service Model Key',
    label: 'Service Model Key',
    fieldType: 'hidden',
    type: 'hidden',
    value: V24_SAAS_SERVICE_MODEL_KEY,
    visibility: 'internal',
    ...extra
  };
...
export function canPostCreateChangeClassification(value = {}, role = 'client', field = 'severity') {
  if (!isV24SaasRequest(value)) return true; // leave pre-v24 / normal logic alone
  const normalizedRole = String(role || '').toLowerCase();
  const normalizedField = String(field || '').toLowerCase();
  // v24.2: client users may correct Severity after creation. Priority is support-only and hidden.
  if (normalizedRole === 'client') return normalizedField === 'severity';
  return normalizedField === 'severity' || normalizedField === 'priority';
}
...
export function syncStatusAcrossSupportStages(requestItem = {}, nextStatus = {}) {
  if (!isV24SaasRequest(requestItem)) return requestItem;
...
export function assertNormalIncidentUntouched(before = {}, after = {}) {
  if (isV24SaasRequest(before) || isV24SaasRequest(after)) return true;
  ...
  for (const key of keys) {
    ...
    if (a !== b) throw new Error(`Normal incident guard failed: ${key} changed without ${V24_SAAS_SERVICE_MODEL_KEY}.`);
  }
  return true;
}
...
export function matchForm(forms = [], value = {}) {
  if (!isV24SaasRequest(value)) return null;
