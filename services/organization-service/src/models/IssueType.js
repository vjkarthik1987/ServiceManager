 ...
 * level 3 = Subtype
 *
 * A single collection is intentionally retained because the three objects share
 * ordering, status and behaviour references. parentTypeId enforces the hierarchy
 * at the service/API layer.
 */
const issueTypeSchema = new mongoose.Schema(
  {
    ...
    // Behaviour references can be defined on Issue Type (L2) or Subtype (L3).
    // Subtype values override the parent Issue Type in the v23.1 intake layer.
    workflowId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'Workflow',
      default: null,
...
issueTypeSchema.index({ organizationId: 1, level: 1, parentTypeId: 1, displayOrder: 1, name: 1 });
issueTypeSchema.index({ organizationId: 1, parentTypeId: 1, status: 1, displayOrder: 1 });
