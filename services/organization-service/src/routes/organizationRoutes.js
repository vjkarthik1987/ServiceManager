...
function enrichSupportPathWithWorkflows(supportPath, workflowsById = new Map()) {
  ...
  item.levels = (item.levels || []).map((level) => {
    ...
    return {
      ...level,
      workflow,
      workflowDefinition: workflow ? { statuses: workflow.statuses || [], transitions: workflow.transitions || [], globalActions: workflow.globalActions || [], sourceMetadata: workflow.sourceMetadata || {} } : { statuses: [], transitions: [], globalActions: [], sourceMetadata: {} }
    };
  });
  return item;
}
