...
async function createIssueType({ organizationId, level, parentTypeId = null, name, description = '', icon = '◌', displayOrder = 100, fieldsConfig = null, customFields = [] }) {
...
function buildTree(types, workflowsById = new Map(), supportPathsById = new Map()) {
  const level1 = types.filter((item) => item.level === 1);
  const level2 = types.filter((item) => item.level === 2);

  return level1.map((parent) => ({
    ...parent.toObject(),
    children: level2
      .filter((child) => String(child.parentTypeId) === String(parent._id))
      .map((child) => {
...
organizationRouter.get('/:organizationId/issue-types', async (req, res, next) => {
  try {
    ...
    res.json({ issueTypes: types, tree: buildTree(types, workflowsById, supportPathsById) });
  } catch (error) {
    next(error);
  }
});
...
organizationRouter.post('/:organizationId/issue-types/level2', async (req, res, next) => {
  try {
    ...
    if (!isValidId(parentTypeId)) return res.status(400).json({ message: 'Valid Level 1 issue type is required.' });

    const parent = await IssueType.findOne({ _id: parentTypeId, organizationId: organization._id, level: 1 });
    if (!parent) return res.status(404).json({ message: 'Parent Level 1 issue type not found.' });

    const firstSubtype = await IssueType.findOne({ organizationId: organization._id, level: 2, parentTypeId: parent._id }).sort({ displayOrder: 1, createdAt: -1 });
    const issueType = await createIssueType({
      organizationId: organization._id,
      level: 2,
...
organizationRouter.post('/:organizationId/issue-types/:issueTypeId', async (req, res, next) => {
  try {
    ...
    if (issueType.level === 2) issueType.fieldsConfig = normalizeIssueTypeFieldsConfig(req.body, issueType.fieldsConfig || {});
...
organizationRouter.post('/:organizationId/clients/:clientId/availability', async (req, res, next) => {
  try {
    ...
    if (requestedMode === 'inherit') {
      ...
      client.enabledLevel1IssueTypeIds = [];
      // A child that inherits family availability should inherit family-specific SLA choices as well.
      client.familySlaAssignments = [];
    } else {
      const requestedIds = [...new Set(Array.isArray(req.body.enabledLevel1IssueTypeIds) ? req.body.enabledLevel1IssueTypeIds : [])].map(String);
      ...
      client.enabledLevel1IssueTypeIds = validIds;
...
organizationRouter.post('/:organizationId/clients/:clientId/sla-family', async (req, res, next) => {
  try {
    ...
    const enabled = new Set((client.enabledLevel1IssueTypeIds || []).map(String));
