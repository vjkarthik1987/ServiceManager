#!/usr/bin/env node
/**
 * Service Manager v24.2.4 — controlled SunTecGroup service-model migration.
 *
 * Purpose
 * -------
 * Moves the EXISTING suntecgroup operational workflows onto the v24.2.4
 ...
 * - No deletes.
 * - Existing requests are NOT rewritten by this script. New requests use v24.2.4 configuration.
 * - Use sync-open-requests-to-v24.2.4-workflows.mjs separately for open UAT requests.
 *
 * Usage
 * -----
 *   node scripts/migrate-suntecgroup-to-v24.2.4-workflows.mjs
 *   node scripts/migrate-suntecgroup-to-v24.2.4-workflows.mjs --workspace=suntecgroup --dry-run
 *   node scripts/migrate-suntecgroup-to-v24.2.4-workflows.mjs --workspace=suntecgroup --apply
 *
 * Optional:
...
PROBLEM + CHANGE REQUEST`);
    ...
    if (APPLY) {
      ...
      const reportFile = path.resolve(process.cwd(), `v24.2.4-migration-${WORKSPACE}-${timestamp()}.json`);
      ...
      log(`  node scripts/migrate-suntecgroup-to-v24.2.4-workflows.mjs --workspace=${WORKSPACE} --apply`);
    }

    log(`\nImportant:`);
    log(`  • Existing open requests keep their current embedded workflow snapshots.`);
    log(`  • New Incident/Problem/Change Request/Maintenance Request records use v24.2.4 configuration after migration.`);
