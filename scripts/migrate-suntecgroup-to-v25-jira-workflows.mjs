#!/usr/bin/env node
/**
 * Service Manager v25.0.0 — controlled SunTecGroup service-model migration.
 *
 * Purpose
 * -------
 * Moves the EXISTING suntecgroup operational workflows onto the v25.0.0
 * live-Jira-source model without deleting historical configuration or touching users,
 * clients, requests, comments, audit history, products/modules, or SLA rules.
 *
 * What it changes
 * ---------------
 * 1. Upserts the live-Jira-source Incident, Problem, Change Request and Maintenance Request workflows.
 * 2. Upserts the v24 common L1/L2/L3 Incident support path.
 ...
 * - No deletes.
 * - Existing requests are NOT rewritten by this script. New requests use v25.0.0 configuration.
 * - Use sync-open-requests-to-v25-workflows.mjs separately for open UAT requests.
 *
 * Usage
 * -----
 *   node scripts/migrate-suntecgroup-to-v25-jira-workflows.mjs
 *   node scripts/migrate-suntecgroup-to-v25-jira-workflows.mjs --workspace=suntecgroup --dry-run
 *   node scripts/migrate-suntecgroup-to-v25-jira-workflows.mjs --workspace=suntecgroup --apply
 *
 * Optional:
...
if (manifest.key !== V24_MODEL_KEY || manifest.version !== V25_VERSION) {
  throw new Error(`Expected ${V24_MODEL_KEY} ${V25_VERSION} config.`);
}
...
function mapStatus(item, index) {
  ...
  return {
    ...
    taskTemplates: [],
    jiraStatusId: String(item.jiraStatusId || '').trim(),
    jiraStatusCategory: String(item.jiraStatusCategory || '').trim(),
    approvalConfiguration: item.approvalConfiguration || null,
    sourceProperties: item.sourceProperties || {},
    jiraDescription: String(item.jiraDescription || '').trim()
  };
}
...
function mapTransition(item) {
  return {
    ...
    clientEnabled: item.clientEnabled === true,
    jiraCustomerEnabled: item.jiraCustomerEnabled === true,
...
function buildWorkflowDocument(organizationId) {
  return {
    ...
    description: String(incidentWorkflowConfig.description || 'Common v24 live-Jira-source Incident workflow.').slice(0, 420),
    ...
    sourceMetadata: incidentWorkflowConfig.jiraSource || {},
    status: 'active'
  };
}
...
async function upsertIncidentWorkflow(orgId) {
  ...
  const changed = !same(
    { name: workflow.name, description: workflow.description, statuses: workflow.statuses, transitions: workflow.transitions, globalActions: workflow.globalActions, sourceMetadata: workflow.sourceMetadata, status: workflow.status },
    { name: desired.name, description: desired.description, statuses: desired.statuses, transitions: desired.transitions, globalActions: desired.globalActions, sourceMetadata: desired.sourceMetadata, status: desired.status }
  );
...
async function upsertV24ConfigurationStore(orgId, maintenance) {
  ...
  for (const [kind, key, payload] of configDocs) {
    log(`  ${APPLY ? '✓' : '~'} ${kind} @ ${V25_VERSION}`);
    if (APPLY) {
      await configCollection.updateOne(
        { serviceModelKey: V24_MODEL_KEY, kind, key },
        { $set: { serviceModelKey: V24_MODEL_KEY, kind, key, version: V25_VERSION, payload, updatedAt: new Date() }, $setOnInsert: { createdAt: new Date() } },
        { upsert: true }
      );
    }
  }
  ...
  for (const [canonical, subtype] of maintenance.subtypes.entries()) {
    ...
    if (APPLY) {
      ...
      await rawTypes.updateOne(
        { _id: subtype._id },
        { $set: { serviceModelKey: V24_MODEL_KEY, v24FormKey: formKey, v24WorkflowKey: MAINTENANCE_WORKFLOW_KEY, v24ApprovalPolicyKey: '', v24ConfigVersion: V25_VERSION } }
      );
    }
  }
}
...
async function verify(orgId, taxonomyIds) {
  ...
  if (supportPath) {
    ...
    for (const required of ['request_l2', 'escalate_l3']) if (!actionIds.has(required)) errors.push(`support path missing action ${required}`);
  }
...
async function main() {
  ...
  log(`Target    : ${V24_MODEL_KEY} ${V25_VERSION}`);
...
PROBLEM + CHANGE REQUEST`);
    ...
    if (APPLY) {
      ...
      const reportFile = path.resolve(process.cwd(), `v25-migration-${WORKSPACE}-${timestamp()}.json`);
      ...
      log(`  node scripts/migrate-suntecgroup-to-v25-jira-workflows.mjs --workspace=${WORKSPACE} --apply`);
    }

    log(`\nImportant:`);
    log(`  • Existing open requests keep their current embedded workflow snapshots.`);
    log(`  • New Incident/Problem/Change Request/Maintenance Request records use v25.0.0 configuration after migration.`);
