#!/usr/bin/env node
/**
 * Service Manager v25.0.0 — refresh workflow snapshots on EXISTING open UAT requests.
 *
 ...
 *
 * Usage:
 *   node scripts/sync-open-requests-to-v25-workflows.mjs --workspace=suntecgroup
 *   node scripts/sync-open-requests-to-v25-workflows.mjs --workspace=suntecgroup --apply
 * Optional:
 *   --backup-dir=./backups/v25-open-request-sync
 *   --include-resolved
 */
...
const BACKUP_ROOT = path.resolve(process.cwd(), valueArg('--backup-dir', './backups/v25-open-request-sync'));
...
async function main() {
  ...
  try {
    ...
    for (const key of workflowKeys) if (!workflows.has(key)) throw new Error(`Missing active workflow ${key}. Run the v25.0.0 configuration migration first.`);
    const incidentPath = await SupportPath.findOne({ organizationId: org._id, key: INCIDENT_PATH_KEY, status: 'active' }).lean();
    if (!incidentPath) throw new Error(`Missing active support path ${INCIDENT_PATH_KEY}. Run the v25.0.0 configuration migration first.`);
