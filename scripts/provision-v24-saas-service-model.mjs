#!/usr/bin/env node
/**
 * Service Manager v24 — SunTec SaaS service model provisioner.
 *
 * SAFE BY DEFAULT:
 *   node scripts/provision-v24-saas-service-model.mjs --dry-run
 *
 * APPLY SAFE CONFIG ONLY:
 *   node scripts/provision-v24-saas-service-model.mjs --apply
 *
 * APPLY EXPLICIT AMBIGUOUS TYPE BINDINGS AFTER DRY-RUN REVIEW:
 *   node scripts/provision-v24-saas-service-model.mjs --apply --approve-bindings=collection:id,collection:id
 *
...
const EXPLICIT_MONGO_URI = valueArg('--mongo-uri') || process.env.V24_MONGO_URI || '';
const CONFIG_COLLECTION = process.env.V24_SERVICE_MODEL_COLLECTION || 'v24_service_models';
...
if (manifest.key !== V24_SAAS_SERVICE_MODEL_KEY) throw new Error('Invalid v24 service-model manifest.');
...
function hasSaasEvidence(doc = {}) {
  if (String(doc.serviceModelKey || '') === V24_SAAS_SERVICE_MODEL_KEY) return true;
...
const BACKUP_DIR = path.resolve(process.cwd(), 'backups', 'v24-pre-migration', backupName());
...
async function main() {
  ...
  print(`Service Manager v24 SaaS provisioner — ${DRY_RUN ? 'DRY RUN' : 'APPLY'}`);
  print(`Service model: ${V24_SAAS_SERVICE_MODEL_KEY}`);
  ...
  if (LEGACY_CONFIRM_CATALOGUE && !APPROVED_BINDINGS.size) {
    throw new Error('--confirm-current-saas-catalogue is intentionally disabled in v24. Use --approve-bindings=collection:id after reviewing the dry-run.');
  }
  ...
  try {
    ...
    throw new Error('Database preflight failed. No v24 database writes were attempted.');
  }
  ...
  try {
    ...
    if (unsafeGeneric.length) throw new Error('Guard violation: a generic request-family type was selected for v24 binding.');
    ...
    const snapshot = {
      ...
      serviceModelKey: V24_SAAS_SERVICE_MODEL_KEY,
    ...
    if (APPLY && matches.length) {
      ...
      fs.writeFileSync(path.join(BACKUP_DIR, 'taxonomy-before-v24.json'), JSON.stringify(snapshot, null, 2));
    }
...
async function upsertConfiguration(collection) {
  ...
  for (const [kind, key, payload] of docs) {
    const filter = { serviceModelKey: V24_SAAS_SERVICE_MODEL_KEY, kind, key };
    const current = await collection.findOne(filter);
    const next = { ...filter, version: manifest.version, payload, updatedAt: new Date() };
    if (!APPLY) {
...
async function bindType(db, configCollection, row) {
  ...
  const update = {
    $set: {
      serviceModelKey: V24_SAAS_SERVICE_MODEL_KEY,
      v24FormKey: form.key,
      v24WorkflowKey: form.workflowKey,
      v24ApprovalPolicyKey: form.approvalPolicyKey || '',
      v24ConfigVersion: manifest.version
    }
  };
...
async function migrateOpenSaasRequests(db, configCollection) {
  const bindings = await configCollection.find({ serviceModelKey: V24_SAAS_SERVICE_MODEL_KEY, kind: 'type_binding' }).toArray();
  ...
  for (const name of requestCollections) {
    ...
    for (const doc of docs) {
      ...
      const migrationLog = path.join(BACKUP_DIR, 'open-requests-before-v24.ndjson');
      fs.appendFileSync(migrationLog, `${JSON.stringify({ collection: name, document: before })}\n`);
      const result = await collection.updateOne({ _id: doc._id }, { $set: { customFields, v24ConfigVersion: manifest.version } });
      if (result.modifiedCount) stats.updated++; else stats.unchanged++;
    }
  }
}
...
async function provisionProductVersionMetadata(db) {
  ...
  const filter = { serviceModelKey: V24_SAAS_SERVICE_MODEL_KEY, kind: 'schema' };
  if (!APPLY) { stats.updated++; return; }
  const result = await collection.updateOne(filter, { $set: { ...filter, version: manifest.version, model: productVersionModel, updatedAt: new Date() }, $setOnInsert: { createdAt: new Date() } }, { upsert: true });
  if (result.upsertedCount) stats.created++; else if (result.modifiedCount) stats.updated++; else stats.unchanged++;
}

main().catch((error) => {
  stats.errors++;
  console.error(`v24 provisioner failed: ${error.stack || error.message}`);
  process.exitCode = 1;
});
