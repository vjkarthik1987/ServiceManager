#!/usr/bin/env node
/**
 * Service Manager v24.2.0 — controlled SunTecGroup service-model migration.
 *
 * Purpose
 * -------
...
function customFieldFromForm(formField, index) {
  ...
  return {
    ...
    fieldType: typeMap[registry.type] || 'short_text',
...
async function main() {
  ...
  log(`Scope     : Incident + Problem + Change Request configuration; no request/history/user/client deletes or rewrites.\n`);
...
PROBLEM + CHANGE REQUEST`);
    ...
    if (APPLY) {
      ...
      const reportFile = path.resolve(process.cwd(), `v24.2-migration-${WORKSPACE}-${timestamp()}.json`);
      ...
      log(`  node scripts/migrate-suntecgroup-to-v24.2.mjs --workspace=${WORKSPACE} --apply`);
    }

    log(`\nImportant:`);
    log(`  • Existing open requests keep their current embedded workflow snapshots.`);
    log(`  • New Incident/Problem/Change Request records use v24.2 configuration after migration.`);
    log(`  • Old workflow/path documents are preserved for rollback and audit.`);
    log(`  • Service Request and Maintenance Request configuration is untouched. Components LOV is provisioned on all Incident subtypes.`);
  } finally {
