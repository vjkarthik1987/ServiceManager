#!/usr/bin/env node
/**
 * Service Manager v24.2.3 — controlled SunTecGroup service-model migration.
 *
 * Purpose
 * -------
 * Moves the EXISTING suntecgroup operational workflows onto the v24.2.3
 * JSM-aligned model without deleting historical configuration or touching users,
 * clients, requests, comments, audit history, products/modules, or SLA rules.
 *
 * What it changes
 * ---------------
 * 1. Upserts the JSM-aligned Incident, Problem, Change Request and Maintenance Request workflows.
 ...
 * - No deletes.
 * - Existing requests are NOT rewritten by this script. New requests use v24.2.3 configuration.
 * - Use sync-open-requests-to-v24.2.3-workflows.mjs separately for open UAT requests.
 *
 * Usage
 * -----
 *   node scripts/migrate-suntecgroup-to-v24.2.3-workflows.mjs
 *   node scripts/migrate-suntecgroup-to-v24.2.3-workflows.mjs --workspace=suntecgroup --dry-run
 *   node scripts/migrate-suntecgroup-to-v24.2.3-workflows.mjs --workspace=suntecgroup --apply
 *
 * Optional:
...
async function verify(orgId, taxonomyIds) {
  ...
  if (supportPath) {
    ...
    for (const required of ['request_l2', 'assign_l3', 'escalate_l3']) if (!actionIds.has(required)) errors.push(`support path missing action ${required}`);
  }
...
PROBLEM + CHANGE REQUEST`);
    ...
    if (APPLY) {
      ...
      const reportFile = path.resolve(process.cwd(), `v24.2.3-migration-${WORKSPACE}-${timestamp()}.json`);
      ...
      log(`  node scripts/migrate-suntecgroup-to-v24.2.3-workflows.mjs --workspace=${WORKSPACE} --apply`);
    }

    log(`\nImportant:`);
    log(`  • Existing open requests keep their current embedded workflow snapshots.`);
    log(`  • New Incident/Problem/Change Request/Maintenance Request records use v24.2.3 configuration after migration.`);
