...
function normalizedWorkflowConditionToken(value = '') {
  return String(value || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim().replace(/\s+/g, ' ');
}

function workflowRequestTypeCandidatesForUi(request = {}) {
  const family = String(request.level2Type?.name || request.level2Type?.code || '').trim();
  const subtype = String(request.level3Type?.name || request.level3Type?.code || '').trim();
  const candidates = [family, subtype, `${subtype} ${family}`, `${family} ${subtype}`]
    .map(normalizedWorkflowConditionToken)
    .filter(Boolean);
  return [...new Set([
    ...candidates,
    ...candidates.map((value) => value.replace(/\brequest\b/g, '').replace(/\s+/g, ' ').trim()).filter(Boolean)
  ])];
}

function workflowConditionTypeMatchesForUi(expected = '', candidates = []) {
  const wanted = normalizedWorkflowConditionToken(expected);
  if (!wanted) return true;
  const loose = wanted.replace(/\brequest\b/g, '').replace(/\s+/g, ' ').trim();
  return candidates.some((candidate) => candidate === wanted || candidate === loose || candidate.includes(loose) || loose.includes(candidate));
}

function originSupportLevelForUi(request = {}) {
  const explicit = String(request.originSupportLevel || '').trim().toUpperCase();
  ...
  const visibility = String(request.visibilityScope || '').trim().toLowerCase();
  if (visibility === 'client_visible') return 'L1';
  if (visibility === 'partner_visible') return 'L2';
  return String(request.currentSupportLevel || 'L3').trim().toUpperCase();
}

function actorIsReporterForUi(actor = {}, request = {}) {
  const actorId = String(actor?._id || actor?.actorId || '').trim();
  const actorEmail = String(actor?.email || '').trim().toLowerCase();
  return [request.requester || {}, request.raisedOnBehalfOf || {}].some((item) =>
    (actorId && String(item.actorId || '').trim() === actorId)
      || (actorEmail && String(item.email || '').trim().toLowerCase() === actorEmail)
  );
}

function workflowConditionAllowedForUi(condition = {}, { portal = '', assignment = null, actor = {}, request = {}, stage = null } = {}) {
  if (!condition || typeof condition !== 'object') return true;
  const candidates = workflowRequestTypeCandidatesForUi(request);
  const requestTypes = Array.isArray(condition.requestTypes) ? condition.requestTypes : [];
  if (requestTypes.length && !requestTypes.some((value) => workflowConditionTypeMatchesForUi(value, candidates))) return false;
  const excludedTypes = Array.isArray(condition.excludedRequestTypes) ? condition.excludedRequestTypes : [];
  if (excludedTypes.some((value) => workflowConditionTypeMatchesForUi(value, candidates))) return false;

  const originLevel = originSupportLevelForUi(request);
  const allowedOrigins = Array.isArray(condition.originSupportLevels) ? condition.originSupportLevels.map((value) => String(value || '').toUpperCase()) : [];
  if (allowedOrigins.length && !allowedOrigins.includes(originLevel)) return false;
  const excludedOrigins = Array.isArray(condition.excludedOriginSupportLevels) ? condition.excludedOriginSupportLevels.map((value) => String(value || '').toUpperCase()) : [];
  if (excludedOrigins.includes(originLevel)) return false;

  const currentLevel = String(stage?.localId || request.currentSupportLevel || '').trim().toUpperCase();
  const allowedCurrent = Array.isArray(condition.currentSupportLevels) ? condition.currentSupportLevels.map((value) => String(value || '').toUpperCase()) : [];
  if (allowedCurrent.length && !allowedCurrent.includes(currentLevel)) return false;

  const previousIds = Array.isArray(condition.previousStatusIds) ? condition.previousStatusIds.map((value) => String(value || '').toUpperCase()) : [];
  ...
  if (condition.reporterOnly === true && role === 'client' && !actorIsReporterForUi(actor, request)) return false;
  return true;
}

function configuredActionAllowed(action = {}, { portal = '', assignment = null, actor = {}, request = {}, stage = null } = {}) {
  const role = workflowRoleForUi(portal, assignment, actor);
  // JSM approval transitions are controlled by status approvalConfiguration rather than the
  // normal servicedesk.customer.transition.active property. Translate that governance explicitly:
  // Bank approval is client/admin; other active approval gates are manager/admin.
  const approvalActive = String(stage?.currentStatus?.approvalConfiguration?.active || '').toLowerCase() === 'true';
  const approvalKind = String(action.kind || action.transitionType || '').toLowerCase() === 'approval';
  ...
  const clientEnabled = approvalForBank || (action.clientEnabled !== undefined ? action.clientEnabled === true : action.customerEnabled === true);
  if (role === 'client' && !clientEnabled) return false;
  const allowedSupportLevels = Array.isArray(action.allowedSupportLevels)
    ? action.allowedSupportLevels.map((item) => String(item || '').toUpperCase()).filter(Boolean)
    : [];
  if (allowedSupportLevels.length) {
    const currentLevel = String(stage?.localId || request?.currentSupportLevel || '').toUpperCase();
  ...
  if (!workflowConditionAllowedForUi(action.condition || {}, { portal, assignment, actor, request, stage })) return false;
...
function workflowInitialStatus(workflow) {
  ...
  return {
    ...
    jiraStatusId: start.jiraStatusId || '',
    jiraStatusCategory: start.jiraStatusCategory || '',
    approvalConfiguration: start.approvalConfiguration || null,
    sourceProperties: start.sourceProperties || {},
    jiraDescription: start.jiraDescription || ''
  };
}
...
function workflowDefinitionFrom(workflow = {}) {
  ...
  return {
    statuses,
    transitions: (workflow?.transitions || []).filter((item) => ids.has(item.fromStatusId) && ids.has(item.toStatusId)),
    globalActions: workflow?.globalActions || [],
    sourceMetadata: workflow?.sourceMetadata || workflow?.jiraSource || {}
  };
}
...
function buildRequestPayload({ req, organization, portal, actor, context }) {
  ...
  let initialStatus = workflowInitialStatus(workflow);
  // The live Jira Incident Create transition immediately routes Partner-origin records to
  // L2 Support and SunTec-origin records to L3 Support. Mirror that post-function natively
  // instead of making users click the origin-assignment transition after creation.
  if (isSaasIncidentIntake) {
    const originTargetStatusId = currentSupportLevel === partnerLevel
      ? 'L2_SUPPORT'
      : currentSupportLevel === suntecLevel
        ? 'L3_SUPPORT'
        : '';
    const routedStatus = originTargetStatusId ? workflowStatusByLocalId(workflow, originTargetStatusId) : null;
    if (routedStatus) {
      initialStatus = {
        localId: routedStatus.localId,
        name: routedStatus.name,
        customerLabel: routedStatus.customerLabel || routedStatus.name,
        statusType: routedStatus.statusType || 'normal',
        isCustomerVisible: routedStatus.isCustomerVisible !== false,
        taskTemplates: routedStatus.taskTemplates || [],
        jiraStatusId: routedStatus.jiraStatusId || '',
        jiraStatusCategory: routedStatus.jiraStatusCategory || '',
        approvalConfiguration: routedStatus.approvalConfiguration || null,
        sourceProperties: routedStatus.sourceProperties || {},
        jiraDescription: routedStatus.jiraDescription || ''
      };
    }
  }
  ...
  primaryStage.currentStatus = initialStatus;
  ...
  return {
    ...
    source,
    visibilityScope,
    originSupportLevel: currentSupportLevel,
    currentSupportLevel,
    ownerSide
  };
}
  ...
  try {
    ...
    const stageViews = activeStages.map((rawStage) => {
      ...
      const configuredStatusActions = currentTransitions
        .filter((item) => configuredActionAllowed(item, { portal, assignment: activeAssignment, actor, request, stage }))
        // JSM shows customer-permitted routing transitions (for example Request to L2 Support)
        // in the same workflow action menu as ordinary status actions. Internal users still get
        // cross-level routing in the dedicated Support routing control so reason/ownership rules
        // remain explicit for operational hand-offs.
        .filter((item) => portal === 'client' || !item.supportEffect?.targetLevel || item.supportEffect.targetLevel === stageLevel)
        .map((item) => ({
          ...item,
      ...
      const sourceBackedWorkflow = Boolean(definition.sourceMetadata?.workflowId)
        || (definition.transitions || []).some((item) => String(item.jiraTransitionId || '').trim());
      let options = stageCanAct
        ? ((v23SaasRequest || sourceBackedWorkflow)
      ...
      const stageSupportActions = (portal === 'admin' || portal !== 'client' || baseCanAct)
        ? (supportPathDefinition.movementRules || [])
            ...
            .filter((item) => configuredActionAllowed(item, { portal, assignment: activeAssignment, actor, request, stage }))
        : [];
      const visibleSupportActions = portal === 'client'
        ? stageSupportActions.filter((rule) => !configuredStatusActions.some((action) => {
            const actionTargetLevel = String(action.targetSupportLevel || action.supportEffect?.targetLevel || '').toUpperCase();
            const ruleTargetLevels = [rule.toLevelId, ...(rule.toLevelIds || [])].map((value) => String(value || '').toUpperCase());
            const sameTargetLevel = actionTargetLevel && ruleTargetLevels.includes(actionTargetLevel);
            const sameTargetStatus = !rule.targetStatusId || String(rule.targetStatusId || '').toUpperCase() === String(action.toStatusId || '').toUpperCase();
            return sameTargetLevel && sameTargetStatus;
          }))
        : stageSupportActions;
      return {
        ...stage,
        ...
        statusActions: stageCanAct && (v23SaasRequest || sourceBackedWorkflow) ? configuredStatusActions : [],
        globalActions: stageCanAct
          ? (definition.globalActions || []).filter((item) => configuredActionAllowed(item, { portal, assignment: activeAssignment, actor, request, stage }))
          : [],
        tasks,
        openBlockingTasks: tasks.filter((task) => task.isBlocking && !['done', 'cancelled'].includes(task.status)),
        // Internal cross-level routing stays in its dedicated control. For a client, a route
        // already represented by a JSM-style workflow action is hidden here to avoid duplicate buttons.
...
async function handleStatusChange(req, res, next, portal) {
  try {
    ...
    const expectedFromStatusId = String(req.body.expectedFromStatusId || storedStage?.currentStatus?.localId || stage?.currentStatus?.localId || '').trim().toUpperCase();
    const requestedToStatusId = String(req.body.toStatusId || '').trim().toUpperCase();
    const requestedTransitionKey = String(req.body.transitionKey || '').trim().toUpperCase();
    const selectedTransition = (definition.transitions || []).find((item) =>
      String(item.fromStatusId || '').trim().toUpperCase() === expectedFromStatusId
        && String(item.toStatusId || '').trim().toUpperCase() === requestedToStatusId
        && (!requestedTransitionKey || String(item.localId || '').trim().toUpperCase() === requestedTransitionKey)
        && configuredActionAllowed(item, { portal, assignment, actor, request, stage })
    );
    const transitionFieldValues = (selectedTransition?.requiredFields || [])
      .filter((field) => !['COMMENT', 'ASSIGNEE', 'RESOLUTION'].includes(String(field.key || '').trim().toUpperCase()))
      .map((field) => {
        const key = String(field.key || '').trim().toUpperCase();
        const raw = req.body[`workflowField__${key}`];
        const value = Array.isArray(raw) ? raw.filter((item) => String(item ?? '').trim()) : String(raw ?? '').trim();
        if ((Array.isArray(value) && !value.length) || (!Array.isArray(value) && !value)) return null;
        return {
          fieldKey: key,
    ...
    const transitionTargetLevel = String(selectedTransition?.targetSupportLevel || selectedTransition?.supportEffect?.targetLevel || '').trim().toUpperCase();
    if (portal === 'client' && transitionTargetLevel && transitionTargetLevel !== String(actionLevel || '').trim().toUpperCase()) {
      ...
      const movementRule = (pathDefinition.movementRules || []).find((rule) => {
        const targetLevels = [rule.toLevelId, ...(rule.toLevelIds || [])].map((value) => String(value || '').trim().toUpperCase());
        const allowedFrom = (rule.allowedFromStatusIds || []).map((value) => String(value || '').trim().toUpperCase());
        return String(rule.fromLevelId || '').trim().toUpperCase() === String(actionLevel || '').trim().toUpperCase()
          && targetLevels.includes(transitionTargetLevel)
          && (!allowedFrom.length || allowedFrom.includes(expectedFromStatusId))
          && (!rule.targetStatusId || String(rule.targetStatusId || '').trim().toUpperCase() === requestedToStatusId)
          && configuredActionAllowed(rule, { portal, assignment, actor, request, stage });
      });
      if (!selectedTransition || !movementRule) {
        return res.status(400).render('pages/error', { title: 'Workflow changed', status: 400, message: 'That support-routing transition is no longer available. Refresh the request and try again.' });
      }
      // Reuse the guarded support-move handler so support level, target status, audit and mail
      // all change atomically even though the customer selected the action from Change status.
      req.body.ruleId = movementRule.localId;
      req.body.expectedFromLevelId = actionLevel;
      req.body.reason = req.body.reason || '';
      return handleSupportMove(req, res, next, portal);
    }

    const result = await changeRequestStatus(organization._id, req.params.requestId, {
      stageId: actionLevel,
      toStatusId: req.body.toStatusId,
      transitionKey: selectedTransition?.localId || requestedTransitionKey,
...
async function handleGlobalWorkflowAction(req, res, next, portal) {
  try {
    ...
    if (!action || !configuredActionAllowed(action, { portal, assignment, actor, request, stage })) {
...
async function handleSupportMove(req, res, next, portal) {
  try {
    ...
    const configuredCrossLevelRoute = portal !== 'client'
      ...
      && configuredActionAllowed(requestedRule, { portal, assignment, actor, request, stage: sourceStage });
