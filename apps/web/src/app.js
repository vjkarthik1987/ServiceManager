...
function resolveEffectiveLevel1Ids(client, clientsById, seen = new Set()) {
  ...
  return (client.enabledLevel1IssueTypeIds || []).map(String);
}
...
async function sendRequestMail({ organization, request, event, actor = {}, extra = '' }) {
  if (!request) return null;
  const recipient = (request.visibilityScope || 'client_visible') === 'client_visible'
    ? (request.raisedOnBehalfOf?.email || request.requester?.email || actor.email || 'unknown@suntecgroup.com')
    : (request.requester?.email || actor.email || 'unknown@suntecgroup.com');
...
function currentSupportPathForRequest(request = {}, context = {}) {
  ...
  const configuredPath = configuredLevel2?.supportPath || null;
  if (!configuredPath) return null;
  if (Array.isArray(configuredPath.levels) && configuredPath.levels.length) return configuredPath;
  const configuredId = configuredPath._id || configuredPath.id || configuredLevel2?.supportPathId || '';
...
function selectedRequestCreationState(req, context, portal = 'admin') {
  ...
  const supportResolution = selectedClient && selectedLevel2
    ? resolveEffectiveSupportPath({ client: selectedClient, clientsById: context.clientsById, level2Type: selectedLevel2, supportPaths: context.supportPaths })
  ...
  const workflow = stage?.workflow || selectedLevel2?.workflow || null;
  const initialStatus = workflowInitialStatus(workflow);
  const fieldConfig = issueFieldConfig(selectedLevel2 || {});
  const customFieldDefinitions = activeCustomFields(selectedLevel2 || {});
  ...
  const effectiveSlaPolicy = effectiveSlaPolicyId ? findById(context.slaPolicies || [], effectiveSlaPolicyId) : null;
  ...
  if (!showClientStep || selectedClient) currentStep = 'type';
  if (selectedClient && selectedLevel1) currentStep = 'subtype';
  if (selectedClient && selectedLevel1 && selectedLevel2) currentStep = 'details';
...
function buildRequestPayload({ req, organization, portal, actor, context }) {
  ...
  if (!level2Type) {
    const error = new Error('Select a valid Level 2 issue type.');
    error.status = 400;
    throw error;
  }

  const fallbackWorkflow = level2Type.workflow || null;
  const fieldConfig = issueFieldConfig(level2Type);
  const customFieldValues = activeCustomFields(level2Type).map((field) => customFieldValueFromBody(field, req.body)).filter(Boolean);
  ...
  const severity = fieldConfig.severity ? findById(context.severities, req.body.severityId) : null;
...
app.get('/', async (req, res) => {
  try {
    ...
    return res.redirect(latest?.organization?.status === 'active' ? '/admin/login' : '/setup');
  } catch {
    return res.render('pages/setup', {
      title: 'Create your organization',
      form: {},
      errorMessage: 'The backend services are not reachable. Start the services and try again.',
      serviceIssue: true
    });
  }
});
...
app.post('/:tenant/login', async (req, res, next) => {
  ...
  try {
    ...
    res.redirect(portalHomePath(tenantPortal, organizationSlug(orgResponse.organization)));
  } catch (error) {
    try {
      const { organization } = await getOrganizationByWorkspace(tenantSlug);
...
function requestFieldValue(item, field) {
  const map = {
    ...
    type: item.level1Type?.name,
    family: item.level1Type?.name,
    subtype: item.level2Type?.name,
...
app.get('/issue-types', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
    const [{ tree }, { workflows }, { supportPaths }] = await Promise.all([listIssueTypes(organization._id), listWorkflows(organization._id), listSupportPaths(organization._id)]);
...
app.get('/issue-types/families/:familyId', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
    const [{ tree }, { workflows }, { supportPaths }] = await Promise.all([
      listIssueTypes(organization._id),
      listWorkflows(organization._id),
      listSupportPaths(organization._id)
    ]);
    const family = (tree || []).find((item) => String(item._id) === String(req.params.familyId));
    if (!family) return res.status(404).render('pages/error', { title: 'Issue family not found', status: 404, message: 'This issue family no longer exists.' });
...
app.get('/issue-types/level2/:issueTypeId/fields', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
    const [{ tree }] = await Promise.all([listIssueTypes(organization._id)]);
    let parentType = null;
    let issueType = null;
    for (const parent of tree || []) {
      const found = (parent.children || []).find((child) => String(child._id) === String(req.params.issueTypeId));
      if (found) {
        parentType = parent;
        issueType = found;
        break;
    ...
    if (!issueType) return res.status(404).render('pages/error', { title: 'Not found', message: 'Level 2 issue type not found.' });
...
app.post('/issue-types/level2', async (req, res, next) => {
  try {
    ...
    const fallback = `/admin/issue-types?openFamily=${issueType.parentTypeId}&createdSubtype=${issueType._id}`;
...
app.post('/issue-types/level2/:issueTypeId/workflow', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
    await assignWorkflowToLevel2(organization._id, req.params.issueTypeId, req.body.workflowId || '');
    res.redirect(safeRedirectTarget(req.body.returnTo, '/admin/issue-types'));
  } catch (error) {
    next(error);
  }
...
app.post('/issue-types/level2/:issueTypeId/fields-config', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
    await updateIssueType(organization._id, req.params.issueTypeId, req.body);
    res.redirect(`/admin/issue-types/level2/${req.params.issueTypeId}/fields`);
  } catch (error) { next(error); }
});

app.post('/issue-types/level2/:issueTypeId/custom-fields', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
...
app.post('/issue-types/level2/:issueTypeId/custom-fields/:fieldKey', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
...
app.post('/issue-types/level2/:issueTypeId/support-path', async (req, res, next) => {
  try {
    const organization = await ensureWorkspace(req, res);
    if (!organization) return res.redirect('/admin/login');
    await assignSupportPathToLevel2(organization._id, req.params.issueTypeId, req.body.supportPathId || '');
    res.redirect(safeRedirectTarget(req.body.returnTo, '/admin/issue-types'));
  } catch (error) { next(error); }
});
...
app.post('/clients/:clientId/availability', async (req, res, next) => {
  try {
    ...
    await updateClientAvailability(organization._id, req.params.clientId, {
      ...
      enabledLevel1IssueTypeIds: toArray(req.body.level1TypeIds)
    });
    res.redirect(`/admin/clients/${req.params.clientId}`);
  } catch (error) {
    next(error);
...
async function renderRequestList(req, res, next, { organization, actor, portal = 'admin' }) {
  try {
    ...
    const visibleRequests = (requestResponse.requests || []).filter((item) => {
      if (portal === 'admin') return true;
      const assignment = bestAssignmentForClient(actor, item.client?.id, context.clients, portal);
      return assignmentCanSeeRequest(assignment, item, portal);
    });
...
async function renderRequestDetail(req, res, next, { organization, actor, portal = 'admin', requestId }) {
  try {
    ...
    if (!fallbackWorkflow) fallbackWorkflow = configuredLevel2?.workflow || null;
    const supportPath = currentSupportPathForRequest(request, context);
    ...
    const stageViews = activeStages.map((rawStage) => {
      ...
      const stageCanAct = portal === 'admin' || (baseCanAct && actorOwnsStage);
      const canTakeOwnership = portal !== 'admin' && baseCanAct && !isAssigned && actorIsEligibleForStage;
      ...
      const canAssignOthers = portal === 'admin' || (baseCanAct && actorIsEligibleForStage);
...
async function handleStageAssignment(req, res, next, portal) {
  try {
    ...
    const assignedTo = selected
      ...
      : {};
    await assignRequestStage(organization._id, request._id, stageId, {
      assignedTo,
...
async function handleStatusChange(req, res, next, portal) {
  try {
    ...
    if (!canWorkSupportStage({ portal, actor, assignment, stage, level: actionLevel })) return res.status(403).render('pages/error', { title: 'Action not allowed', status: 403, message: 'Take ownership of this support stage, or ask a manager to assign it, before changing its workflow status.' });
    const configuredLevel2 = configuredLevel2ForRequest(request, context);
    ...
    if (!result.noChange) {
      dispatchRequestMail({ organization, request: result.request, event: 'status changed', actor: { name: actor.name, email: actor.email, role: assignment?.role || actor.userType, portal }, extra: `${actionLevel} status changed.` });
    }
...
async function handleSupportMove(req, res, next, portal) {
  try {
    ...
    if (!canWorkSupportStage({ portal, actor, assignment, stage: sourceStage, level: sourceLevel })) return res.status(403).render('pages/error', { title: 'Action not allowed', status: 403, message: 'Take ownership of this support stage, or ask a manager to assign it, before routing it to another support level.' });
...
async function handleAcknowledgeRequest(req, res, next, portal) {
  try {
    ...
    const { request } = await getServiceRequest(organization._id, req.params.requestId);
    const assignment = portal === 'admin' ? null : bestAssignmentForClient(actor, request.client?.id, context.clients, portal);
    const acknowledgeLevel = request.currentSupportLevel || 'L1';
    const storedAcknowledgeStage = (request.activeStages || []).find((stage) => stage.isPrimary) || (request.activeStages || []).find((stage) => stage.localId === acknowledgeLevel) || null;
    const acknowledgeStage = effectiveStageAgainstPath(storedAcknowledgeStage, currentSupportPathForRequest(request, context));
    if (portal !== 'admin' && !canWorkSupportStage({ portal, actor, assignment, stage: acknowledgeStage, level: acknowledgeLevel })) {
      return res.status(403).render('pages/error', { title: 'Action not allowed', status: 403, message: 'Take ownership of the active support stage before acknowledging this request.' });
    }
...
async function handleAddRequestComment(req, res, next, portal) {
  try {
    ...
    dispatchRequestMail({ organization, request: response.request, event: 'comment added', actor: { name: actor.name, email: actor.email, role: assignment?.role || actor.userType, portal }, extra: `New ${visibility.replace('_', ' ')} comment: ${String(req.body.body || '').slice(0, 500)}` });
    if (wantsJson(req)) {
...
app.post('/:portal(admin|client|agent)/requests', async (req, res, next) => {
  ...
  try {
    ...
    try {
      const { organization, actor, allowed } = await resolvePortalAccess(req, portal);
      if (!allowed) return res.redirect(portalLoginPath(portal));
      req.query = { clientId: req.body.clientId, level1TypeId: req.body.level1TypeId, level2TypeId: req.body.level2TypeId };
