...
test('customer and support routing transitions mirror the JSM permissions', () => {
  assert.equal(transition('START_ANALYSIS_L1').customerEnabled, true);
  assert.equal(transition('REQUEST_L2_NEW').customerEnabled, true);
  assert.equal(transition('ASSIGN_L2').customerEnabled, false);
  assert.equal(transition('ASSIGN_L3').customerEnabled, false);
  assert.equal(manifest.routing.clientDirectL3, false);

  assert.equal(movement('request_l2').customerEnabled, true);
  assert.equal(movement('assign_l2').customerEnabled, false);
  assert.equal(movement('assign_l3').customerEnabled, false);
  assert.deepEqual(movement('assign_l3').condition.severityCodes, ['S1']);
  assert.equal(movement('assign_l3').targetStatusId, 'L3_SUPPORT');
  assert.equal(movement('assign_l2').targetStatusId, 'L2_SUPPORT');
});

test('Request to L2 is available from the four JSM source states', () => {
  assert.deepEqual(movement('request_l2').allowedFromStatusIds, ['NEW','ANALYSIS','IN_PREPRODUCTION','TEST_FAILED']);
});

test('L2 and L3 support have explicit Start Analysis transitions', () => {
  assert.deepEqual([transition('START_L2_ANALYSIS').from, transition('START_L2_ANALYSIS').to], ['L2_SUPPORT','L2_ANALYSIS']);
  assert.deepEqual([transition('START_L3_ANALYSIS').from, transition('START_L3_ANALYSIS').to], ['L3_SUPPORT','L3_ANALYSIS']);
});

test('management escalation is status-preserving and only Support Manager is customer-enabled', () => {
  const actions = new Map(incidentWorkflow.globalActions.map((item) => [item.key, item]));
  assert.equal(actions.get('ESCALATE_SUPPORT_MANAGER').statusEffect, 'KEEP');
  assert.equal(actions.get('ESCALATE_SUPPORT_MANAGER').customerEnabled, true);
  assert.equal(actions.get('ESCALATE_HEAD_SUPPORT').customerEnabled, false);
  assert.equal(actions.get('ESCALATE_GLOBAL_SUPPORT_HEAD').customerEnabled, false);
});
...
test('allowedTransitions enforces customer/role-style workflow metadata', () => {
  const client = allowedTransitions(incidentWorkflow, 'NEW', { role: 'client', subtype: 'Application Incident' });
  assert.ok(client.some((item) => item.key === 'START_ANALYSIS_L1'));
  assert.ok(client.some((item) => item.key === 'REQUEST_L2_NEW'));
  assert.ok(!client.some((item) => item.key === 'ASSIGN_L3'));
});
