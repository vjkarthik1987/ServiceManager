...
test('Maintenance workflow contains JSM states and branch creation actions', () => {
  ...
  for (const label of ['Start Analysis','Approve','Reject','Reopen Request','Cancel Request','Complete','Create Incident','Create Problem','Service Request']) assert.ok(labels.has(label), `missing transition ${label}`);
});
...
test('Maintenance mappings are runtime-bindable and SLA remains inapplicable', () => {
  ...
  assert.ok(['24.2.2','24.2.3','24.2.4','25.0.0'].includes(manifest.version));
