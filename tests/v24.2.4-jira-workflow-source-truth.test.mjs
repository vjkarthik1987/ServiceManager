...
test('v24.2.4 declares live Jira REST exports as workflow authority', () => {
  const manifest = json('config/v24-saas/service-model.json');
  assert.ok(['24.2.4','25.0.0'].includes(manifest.version));
  assert.match(String(manifest.ui?.workflowAlignment || ''), /Live Jira REST workflow exports/i);
  assert.equal(workflowsDoc.source?.release, manifest.version);
...
test('release has dry-run/apply migration and open-request sync for v24.2.4', () => {
  const migration = read('scripts/migrate-suntecgroup-to-v24.2.4-jira-workflows.mjs');
  const sync = read('scripts/sync-open-requests-to-v24.2.4-workflows.mjs');
  assert.match(migration, /const V24_VERSION = '24\.2\.4'/);
  assert.match(migration, /sourceMetadata/);
  assert.match(migration, /DRY RUN is the default/);
  assert.match(sync, /const VERSION = '24\.2\.4'/);
